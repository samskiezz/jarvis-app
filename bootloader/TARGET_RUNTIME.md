...
| Field | Value |
|-------|-------|
| **Future service name** | `boot-sequence (JVM)` |
| **Production language** | Java/JVM |
| **Database / storage owned** | PostgreSQL |
| **Event topic(s) owned** | `control` |
| **API contract** | `contracts/openapi/bootloader-api.yaml` (or noted contract) |
| **Reference implementation (Layer A)** | `(new)` |

## State this plane owns
startup sequence, preflight checks, migration/policy/artefact verification, service registration, safe mode

## The eight questions
- **What state does it own?** startup sequence, preflight checks, migration/policy/artefact verification, service registration, safe mode
- **What database stores it?** PostgreSQL
- **What API exposes it?** `bootloader-api.yaml`
- **What events does it emit?** topic `control` (envelope: `contracts/events/event-envelope.schema.json`)
...
- **What future service does it split into?** `boot-sequence (JVM)`

## Migration path (Layer A → Layer B)
Re-implement `(new)` behind the **same** API + event + table contracts:
