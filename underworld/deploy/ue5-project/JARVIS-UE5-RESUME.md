...
- **Cook WORKS.** ✅ `Packaged/Linux/` built (3.4G: `Underworld.sh` + `Underworld-Linux.pak`, boots into JarvisHUD). It completed in 73s once the box was FREE (build + Linux shaders cached in the shared DDC). KEY LESSON: the only reliable way to cook on this 31GB box is when NO other UE5 cook is running — two cooks share one engine + one UAT mutex + RAM and OOM each other. Cook on a free box (or the Vast 4090).
- **Next = ship + stream** (see below). Open QA: verify the GLB geometry rendered (JarvisHUD pak was only 9.3M; if assets are stubs, re-run `run-jarvis-import.sh`).
