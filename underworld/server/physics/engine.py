...
_LAW_LIST: list[Law] = [
    ...
    # Optics & instruments (expansion #51-56) --------------------------------
    Law("thin_lens", "Thin-lens focal length", "Optics", "physics",
        "1/f = 1/do + 1/di", (Var("do", "m", 0.05, 50), Var("di", "m", 0.05, 50)), "m",
        lambda do, di: 1.0 / (1.0 / do + 1.0 / di)),
    Law("rayleigh_resolution", "Rayleigh angular resolution", "Optics", "physics",
        "theta = 1.22*lambda/D", (Var("lam", "m", 4e-7, 7e-7), Var("D", "m", 1e-3, 10)), "rad",
        lambda lam, D: 1.22 * lam / D),
    Law("critical_angle", "Total internal reflection angle", "Optics", "physics",
        "theta_c = asin(n2/n1)", (Var("n1", "1", 1.3, 2.5), Var("n2", "1", 1.0, 1.29)), "rad",
        lambda n1, n2: math.asin(n2 / n1)),
    Law("beer_lambert", "Beer-Lambert absorbance", "Spectroscopy", "chemistry",
        "A = eps*l*c", (Var("eps", "L/(mol cm)", 10, 1e5), Var("l", "cm", 0.1, 10), Var("c", "mol/L", 1e-6, 1e-2)), "1",
        lambda eps, l, c: eps * l * c),
    Law("photoelectric", "Photoelectric max kinetic energy", "Quantum optics", "quantum",
        "Kmax = h*f - phi", (Var("f", "Hz", 5e14, 3e15), Var("phi", "J", 1e-19, 8e-19)), "J",
        lambda f, phi: K.H * f - phi),
    # Materials & structural failure (expansion #24-30) ----------------------
    Law("griffith_fracture", "Griffith critical stress", "Fracture mechanics", "materials",
        "sigma_c = sqrt(2*E*gamma/(pi*a))", (Var("E", "Pa", 1e9, 4e11), Var("gamma", "J/m^2", 0.1, 50), Var("a", "m", 1e-6, 1e-2)), "Pa",
        lambda E, gamma, a: math.sqrt(2 * E * gamma / (math.pi * a))),
    Law("euler_buckling", "Euler critical buckling load", "Structures", "civil",
        "Pcr = pi^2*E*I/(K*L)^2", (Var("E", "Pa", 1e9, 4e11), Var("I", "m^4", 1e-8, 1e-2), Var("Kf", "1", 0.5, 2), Var("L", "m", 0.5, 20)), "N",
        lambda E, I, Kf, L: math.pi ** 2 * E * I / (Kf * L) ** 2),
    Law("hall_petch", "Hall-Petch yield strength", "Materials", "materials",
        "sigma_y = sigma0 + k*d^-0.5", (Var("sigma0", "Pa", 1e7, 5e8), Var("k", "Pa m^0.5", 1e4, 1e6), Var("d", "m", 1e-7, 1e-4)), "Pa",
        lambda sigma0, k, d: sigma0 + k * d ** -0.5),
    Law("archard_wear", "Archard wear volume", "Tribology", "mechanical",
        "V = K*W*L/H", (Var("Kw", "1", 1e-6, 1e-2), Var("W", "N", 1, 1e4), Var("L", "m", 1, 1e6), Var("H", "Pa", 1e8, 5e10)), "m^3",
        lambda Kw, W, L, H: Kw * W * L / H),
    Law("mohr_coulomb", "Mohr-Coulomb shear strength", "Soil mechanics", "civil",
        "tau = c + sigma_n*tan(phi)", (Var("c", "Pa", 0, 1e5), Var("sigma_n", "Pa", 1e3, 1e6), Var("phi", "rad", 0.1, 0.7)), "Pa",
        lambda c, sigma_n, phi: c + sigma_n * math.tan(phi)),
    # Fluids (expansion #36-39) ----------------------------------------------
    Law("poiseuille", "Hagen-Poiseuille flow", "Fluid dynamics", "fluid",
        "Q = pi*r^4*dp/(8*mu*L)", (Var("r", "m", 1e-4, 0.5), Var("dp", "Pa", 10, 1e6), Var("mu", "Pa s", 1e-4, 1), Var("L", "m", 0.1, 1000)), "m^3/s",
        lambda r, dp, mu, L: math.pi * r ** 4 * dp / (8 * mu * L)),
    Law("stokes_settling", "Stokes terminal velocity", "Fluid dynamics", "fluid",
        "vt = 2*r^2*(rho_p-rho_f)*g/(9*mu)", (Var("r", "m", 1e-6, 1e-2), Var("rho_p", "kg/m^3", 1000, 8000), Var("rho_f", "kg/m^3", 800, 1200), Var("mu", "Pa s", 1e-4, 1)), "m/s",
        lambda r, rho_p, rho_f, mu: 2 * r ** 2 * (rho_p - rho_f) * 9.81 / (9 * mu)),
    Law("cavitation_number", "Cavitation number", "Fluid dynamics", "fluid",
        "Ca = (p-pv)/(0.5*rho*v^2)", (Var("p", "Pa", 1e4, 1e6), Var("pv", "Pa", 1e3, 1e4), Var("rho", "kg/m^3", 800, 1200), Var("v", "m/s", 1, 50)), "1",
        lambda p, pv, rho, v: (p - pv) / (0.5 * rho * v * v)),
    # Advanced / space / quantum (expansion #91-96) --------------------------
    Law("tsiolkovsky", "Rocket equation delta-v", "Astronautics", "physics",
        "dv = ve*ln(m0/mf)", (Var("ve", "m/s", 1000, 5000), Var("m0", "kg", 1000, 1e5), Var("mf", "kg", 100, 999)), "m/s",
        lambda ve, m0, mf: ve * math.log(m0 / mf)),
    Law("kepler_period", "Kepler orbital period", "Orbital mechanics", "physics",
        "T = 2*pi*sqrt(a^3/(G*M))", (Var("a", "m", 7e6, 4e8), Var("M", "kg", 1e22, 2e30)), "s",
        lambda a, M: 2 * math.pi * math.sqrt(a ** 3 / (6.674e-11 * M))),
    Law("debye_length", "Plasma Debye length", "Plasma physics", "physics",
        "lambdaD = sqrt(eps0*kB*T/(n*e^2))", (Var("T", "K", 1e3, 1e7), Var("n", "1/m^3", 1e15, 1e22)), "m",
        lambda T, n: math.sqrt(8.854e-12 * 1.380649e-23 * T / (n * (1.602e-19) ** 2))),
    Law("shockley_diode", "Shockley diode current", "Semiconductors", "electrical",
        "I = Is*(exp(V/(n*Vt))-1)", (Var("Is", "A", 1e-12, 1e-9), Var("V", "V", 0.0, 0.8), Var("nq", "1", 1, 2), Var("Vt", "V", 0.025, 0.026)), "A",
        lambda Is, V, nq, Vt: Is * (math.exp(V / (nq * Vt)) - 1)),
    Law("landauer", "Landauer minimum bit energy", "Information physics", "quantum",
        "Emin = kB*T*ln2", (Var("T", "K", 1, 1000),), "J",
        lambda T: 1.380649e-23 * T * math.log(2)),
    # Remainders — completing the compendium across every section -------------
    Law("von_mises", "von Mises equivalent stress", "Materials", "materials",
        "svm = sqrt(0.5*((s1-s2)^2+(s2-s3)^2+(s3-s1)^2))", (Var("s1", "Pa", 1e6, 1e9), Var("s2", "Pa", 1e6, 1e9), Var("s3", "Pa", 1e6, 1e9)), "Pa",
        lambda s1, s2, s3: math.sqrt(0.5 * ((s1 - s2) ** 2 + (s2 - s3) ** 2 + (s3 - s1) ** 2))),
    Law("avrami", "Avrami transformation fraction", "Phase transformation", "materials",
        "X = 1 - exp(-k*t^n)", (Var("k", "1/s^n", 1e-3, 1), Var("t", "s", 0, 100), Var("n", "1", 1, 4)), "1",
        lambda k, t, n: 1 - math.exp(-k * t ** n)),
    Law("tolerance_stack", "Statistical tolerance stack", "Manufacturing", "mechanical",
        "sigma_tot = sqrt(sum(sigma_i^2))", (Var("s1", "m", 1e-6, 1e-3), Var("s2", "m", 1e-6, 1e-3), Var("s3", "m", 1e-6, 1e-3)), "m",
        lambda s1, s2, s3: math.sqrt(s1 * s1 + s2 * s2 + s3 * s3)),
    Law("fourier_conduction", "Fourier heat conduction", "Heat transfer", "thermodynamics",
        "q = k*A*dT/dx", (Var("k", "W/(m K)", 0.1, 400), Var("A", "m^2", 0.01, 100), Var("dT", "K", 1, 1000), Var("dx", "m", 0.01, 10)), "W",
        lambda k, A, dT, dx: k * A * dT / dx),
    Law("carnot_cop", "Heat-pump coefficient of performance", "Thermodynamics", "thermodynamics",
        "COP = Tc/(Th-Tc)", (Var("Tc", "K", 250, 299), Var("Th", "K", 301, 400)), "1",
        lambda Tc, Th: Tc / (Th - Tc)),
    Law("heat_exchanger", "Heat-exchanger duty", "Heat transfer", "thermodynamics",
        "Q = U*A*dT", (Var("U", "W/(m^2 K)", 10, 5000), Var("A", "m^2", 0.1, 100), Var("dT", "K", 1, 200)), "W",
        lambda U, A, dT: U * A * dT),
    Law("clausius_clapeyron", "Clausius-Clapeyron vapour pressure", "Thermodynamics", "thermodynamics",
        "P2 = P1*exp(-L/R*(1/T2-1/T1))", (Var("P1", "Pa", 1e3, 1e5), Var("L", "J/mol", 2e4, 5e4), Var("T1", "K", 280, 320), Var("T2", "K", 280, 360)), "Pa",
        lambda P1, L, T1, T2: P1 * math.exp(-L / 8.314 * (1 / T2 - 1 / T1))),
    Law("pascal_hydraulic", "Hydraulic force multiplication", "Fluid statics", "fluid",
        "F2 = F1*A2/A1", (Var("F1", "N", 1, 1e4), Var("A1", "m^2", 1e-4, 1e-1), Var("A2", "m^2", 1e-4, 1)), "N",
        lambda F1, A1, A2: F1 * A2 / A1),
    Law("clausius_forcing", "CO2 radiative forcing", "Climate", "physics",
        "dF = 5.35*ln(C/C0)", (Var("C", "ppm", 280, 1200), Var("C0", "ppm", 280, 420)), "W/m^2",
        lambda C, C0: 5.35 * math.log(C / C0)),
    Law("stream_power", "Stream-power erosion", "Geomorphology", "civil",
        "E = K*A^m*S^n", (Var("Kf", "1", 1e-6, 1e-3), Var("A", "m^2", 1, 1e6), Var("m", "1", 0.3, 0.6), Var("S", "1", 1e-3, 0.5), Var("n", "1", 0.5, 1.5)), "m/s",
        lambda Kf, A, m, S, n: Kf * A ** m * S ** n),
    Law("drag_force", "Aerodynamic drag", "Fluid dynamics", "fluid",
        "Fd = 0.5*rho*Cd*A*v^2", (Var("rho", "kg/m^3", 0.5, 1200), Var("Cd", "1", 0.1, 1.5), Var("A", "m^2", 0.1, 100), Var("v", "m/s", 1, 300)), "N",
        lambda rho, Cd, A, v: 0.5 * rho * Cd * A * v * v),
    Law("abbe_resolution", "Abbe diffraction limit", "Optics", "physics",
        "d = lambda/(2*NA)", (Var("lam", "m", 4e-7, 7e-7), Var("NA", "1", 0.1, 1.4)), "m",
        lambda lam, NA: lam / (2 * NA)),
    Law("sabine_reverb", "Sabine reverberation time", "Acoustics", "physics",
        "T60 = 0.161*V/A", (Var("V", "m^3", 10, 1e4), Var("A", "m^2", 1, 1000)), "s",
        lambda V, A: 0.161 * V / A),
    Law("shot_noise", "Shot noise power", "Electronics", "electrical",
        "in^2 = 2*q*I*df", (Var("I", "A", 1e-6, 1), Var("df", "Hz", 1, 1e6)), "A^2",
        lambda I, df: 2 * 1.602e-19 * I * df),
    Law("microgrid_balance", "Microgrid energy balance", "Power systems", "energy",
        "E_next = E + (Pgen-Pload)*dt - losses", (Var("E", "J", 0, 1e8), Var("Pgen", "W", 0, 1e6), Var("Pload", "W", 0, 1e6), Var("dt", "s", 1, 3600), Var("losses", "J", 0, 1e6)), "J",
        lambda E, Pgen, Pload, dt, losses: E + (Pgen - Pload) * dt - losses),
]
