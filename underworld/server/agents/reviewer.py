...
def _heuristic_review(guild_kind: GuildKind, inv: Invention) -> dict[str, Any]:
    ...
    # Physics gate: a proposal that violates a hard physical limit (FTL,
    # over-unity, >100% efficiency) is rejected outright — the world obeys
    # physics. Otherwise the inventor's physics feasibility anchors the score.
    physics = (inv.inputs or {}).get("physics") or {}
    if physics.get("violates_limit"):
        notes = "; ".join(physics.get("notes") or []) or "violates a physical limit"
        return {
            "verdict": "REJECT",
            "rationale": f"Physically impossible: {notes}.",
            "scores": {"feasibility": 0.05, "novelty": 0.1, "safety": 0.8},
        }
    ...
    # Anchor feasibility to the physics assessment when present so grounded,
    # quantified proposals score higher and hand-wavy ones score lower.
    physics_feas = physics.get("feasibility")
