#!/usr/bin/env python3
"""Second-wave custom videos for Jarvis Match Centre.

Generates unique videos for every match card, every group card, and every team
card, skipping files that already exist. Reads source image URLs from the
existing matchcentre_manifest.json so no image credits are burned.

Run with: MATCHCENTRE_VIDEO_BUDGET=1400 .venv/bin/python scripts/generate_matchcentre_videos_wave2.py
"""
...
BUDGET_TOTAL = int(os.environ.get("MATCHCENTRE_VIDEO_BUDGET", "1400"))
CREDIT_COST = 8
CONCURRENCY = 2
...
async def main() -> None:
    ...
    match_by_n = {m["n"]: m for m in fixtures}

    async with httpx.AsyncClient(timeout=300.0) as client:
        tasks: list[asyncio.Task[Any]] = []

        # 1) Every match card (1-104) that doesn't already have a video.
        for n in range(1, 105):
            ...
            prompt = (
                ...
                f"subtle light rays and data particles drifting, {home} versus {away}, "
        ...
        # 2) Every group card (A-L).
        for g in "ABCDEFGHIJKL":
            img_name = f"mc_group_{g.lower()}"
            img_url = image_urls.get(img_name)
            prompt = (
                f"Slow cinematic pan across a glowing World Cup Group {g} standings table, "
                f"national flags and data particles, dark stadium atmosphere, premium broadcast, no text"
            )
            tasks.append(asyncio.create_task(generate_video(
                vid_sem, client, key_id, key_secret, budget, manifest,
                f"mc_group_{g.lower()}_video", prompt, img_url, 5, "16:9", ord(g) % 10000,
            )))

        # 3) Every real team card that has a hero image (skip knockout placeholders).
        ...
        # 4) Panel/header videos for sections, builder, news, and model lab cards.
        panel_jobs = [
