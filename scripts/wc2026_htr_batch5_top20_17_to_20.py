"""HTR batch-5 — top-20 30k-search variants #17..#20 graded vs WC2026 actuals only.

Each variant from `server/data/wc2026_30k_search_results.json` (ranks 17..20)
...
17..20 only. Output is `server/data/wc2026_htr_batch5.json`. The HTR
...
Output:
  - server/data/wc2026_htr_batch5.json
"""
...
logger = logging.getLogger("wc2026_htr_batch5")
...
OUT_JSON = REPO_ROOT / "server" / "data" / "wc2026_htr_batch5.json"

BATCH_RANKS = (17, 18, 19, 20)  # variants this batch evaluates
...
def _audit_log_variant(
    ...
    if is_baseline:
        model_name = "elo_batch5_baseline_wc2026_only"
        run_id = f"htr_batch5_baseline_{ts_tag}"
        notes = (
            "HTR batch-5 paired-bootstrap baseline: production "
        ...
        model_name = f"elo_30k_top{rank:02d}_wc2026_only_batch5"
        run_id = f"htr_batch5_rank{rank:02d}_{ts_tag}"
        p = params
        notes = (
            f"HTR batch-5 (30k-search top-{rank}): walk-forward grade of "
    ...
    try:
        ...
        for (h, a, d) in wc2026_keys:
            pred = {
                ...
                "p_draw": None,
                "p_away": None,
                "source": f"wc2026_htr_batch5.{model_name}",
            }
            try:
                wdb.log_prediction(
                    run_id,
...
def main() -> dict:
    ...
    payload: dict = {
        "verified": True,
        "source": (
            "scripts/wc2026_htr_batch5_top20_17_to_20.py walking 30k-search "
            "variants #17..#20 from server/data/wc2026_30k_search_results.json "
        ...
        "notes": (
            "Batch-5 of the top-20 30k-search evaluation. Each variant's "
