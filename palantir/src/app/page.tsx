...
const FOUNDRY_SEED = { type: 'topic', id: '__ROOT__', label: 'JARVIS ONTOLOGY' };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetchEndpoint('/api/jarvis/geo');   // JARVIS Palantir geo objects (~11.7k+ points)
    fetchEndpoint('/api/jarvis/buoys'); // NOAA NDBC ocean buoy network
    fetchEndpoint('/api/jarvis/acoustic'); // acoustic contacts (general-audio classifier; marine model pending)
    fetchEndpoint('/api/jarvis/fireballs'); // NASA CNEOS fireball airbursts (map layer)
    ...
    const intervals = [
      setInterval(() => fetchEndpoint('/api/jarvis/geo'), 600000),   // 10 min — JARVIS Palantir geo
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── JARVIS title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'JARVIS'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '96px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#D4AF37] font-mono">JARVIS</h1>
            <span className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#D4AF37]">PALANTIR · GLOBAL INTELLIGENCE COMMAND</span>
          </div>
        </div>
...
JARVIS · PALANTIR-CLASS INTELLIGENCE CORE <span className="hidden md:inline">· FOUNDRY: ONTOLOGY · GOTHAM: COMMON OPERATING PICTURE · AIP: REASONING MESH · APOLLO: CONTINUOUS DELIVERY</span>
          </span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-4 text-[9px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <UptimeClock />
        <span className="text-[10px] font-bold tracking-[0.2em] text-[var(--text-muted)] opacity-50 ml-2">JARVIS·PALANTIR</span>
      </motion.div>

      {/* ── MOBILE: Compact top status (upstream donation/token UI removed) ── */}
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        <div className="relative group">
          <button title="Entity link analysis" onClick={() => { setShowEntityGraph(!showEntityGraph); setShowFoundry(false); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${showEntityGraph ? 'bg-[#D4AF37]/20' : 'hover:bg-white/10'}`}>
            <Network className={`w-4 h-4 ${showEntityGraph ? 'text-[var(--gold-primary)]' : 'text-white/60'}`} />
          </button>
        </div>

        <div className="relative group">
          <button title="FOUNDRY — JARVIS ontology brain" onClick={() => { setShowFoundry(!showFoundry); setShowEntityGraph(false); setShowScience(false); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${showFoundry ? 'bg-[#76FF03]/20' : 'hover:bg-white/10'}`}>
            <Database className={`w-4 h-4 ${showFoundry ? 'text-[#76FF03]' : 'text-white/60'}`} />
          </button>
        </div>

        <div className="relative group">
          <button title="SCIENCE — JARVIS science engine consoles" onClick={() => { setShowScience(!showScience); setShowFoundry(false); setShowEntityGraph(false); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${showScience ? 'bg-[var(--cyan-primary)]/20' : 'hover:bg-white/10'}`}>
            <Activity className={`w-4 h-4 ${showScience ? 'text-[var(--cyan-primary)]' : 'text-white/60'}`} />
          </button>
        </div>
      ...
      {/* ── Entity Graph Panel (map-entity intel resolver) ── */}
      {showEntityGraph && (
        <EntityGraphPanel
      ...
      {/* ── FOUNDRY: JARVIS ontology brain (real /v1/graph via /api/jarvis/graph) ── */}
      {showFoundry && (
        <EntityGraphPanel
          entity={FOUNDRY_SEED}
          source="ontology"
