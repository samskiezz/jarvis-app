...
interface Props {
  ...
  onClose: () => void;
  /** 'intel' = map-entity resolver (/api/entity/expand); 'ontology' = JARVIS Foundry brain (/api/jarvis/graph) */
  source?: 'intel' | 'ontology';
}

function EntityGraphPanel({ entity, onClose, source = 'intel' }: Props) {
  ...
  const expandEntity = useCallback(async (type: string, id: string, properties?: Record<string, any>) => {
    ...
    try {
      let url: string;
      if (source === 'ontology') {
        // JARVIS Foundry brain: ontology ids keep their full "type:slug" form.
        url = id === '__ROOT__'
          ? '/api/jarvis/graph'
          : `/api/jarvis/graph?id=${encodeURIComponent(id)}`;
      } else {
  ...
  }, [expandedIds, mergeGraph, source]);

  useEffect(() => {
    if (!entity) return;
    // Ontology (Foundry) mode loads the JARVIS brain neighbourhood directly —
    // no single seed node; the bridge returns the graph.
    if (source === 'ontology') {
    ...
    setError(null);
    expandEntity(entity.type, entity.id, entity.properties);
  }, [entity, source]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleNodeClick = useCallback((node: any) => {
    const n = node as EntityNode;
    setSelectedNode(n);
    // Ontology node ids are already canonical ("topic:slug") — never strip them.
    const rawId = source === 'ontology'
      ? n.id
      : (n.id.includes(':') ? n.id.split(':').slice(1).join(':') : n.id);
    if (!expandedIds.has(`${n.type}:${rawId}`)) expandEntity(n.type, rawId);
  }, [expandedIds, expandEntity, source]);
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--border-primary)] bg-[var(--gold-primary)]/5 relative z-20">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[12px] font-mono font-bold tracking-[0.2em] text-[var(--gold-primary)]">{source === 'ontology' ? '[ JARVIS // FOUNDRY ONTOLOGY ]' : '[ JARVIS // ENTITY INTEL ]'}</span>
        ...
        <AnimatePresence>
          {selectedNode && (
            ...
            >
              ...
              {!expandedIds.has(`${selectedNode.type}:${source === 'ontology' ? selectedNode.id : (selectedNode.id.includes(':') ? selectedNode.id.split(':').slice(1).join(':') : selectedNode.id)}`) && (
                <button onClick={() => {
                  const rawId = source === 'ontology'
                    ? selectedNode.id
                    : (selectedNode.id.includes(':') ? selectedNode.id.split(':').slice(1).join(':') : selectedNode.id);
