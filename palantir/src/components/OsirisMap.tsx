...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core' }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // JARVIS Palantir — the ontology geo objects (cyan core, gold rim)
      map.addLayer({ id: 'jarvis-geo-dots', type: 'circle', source: 'jarvis-geo', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 0,3.5, 4,5, 8,7, 12,10],
        'circle-color': ['match', ['get','type'],
          'SpeciesOccurrence', '#00E676',
          'Sensor', '#00E5FF',
          'Measurement', '#FFB300',
          'Place', '#B388FF',
          'Asset', '#D4AF37',
          '#90A4AE'],
        'circle-opacity': 0.75, 'circle-blur': 0.2,
        'circle-stroke-width': 0.4, 'circle-stroke-color': '#000', 'circle-stroke-opacity': 0.4,
      }});

      // NOAA ocean buoys (DART tsunami buoys red, standard cyan)
      map.addLayer({ id: 'buoy-glow', type: 'circle', source: 'buoys', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,4, 6,9], 'circle-color': '#26C6DA', 'circle-opacity': 0.1, 'circle-blur': 0.8,
      }});
      ...
      map.addLayer({ id: 'fireball-glow', type: 'circle', source: 'fireballs', paint: {
        'circle-radius': ['interpolate',['linear'],['get','energy'], 0,6, 50,18, 500,30], 'circle-color': '#FFD400', 'circle-opacity': 0.12, 'circle-blur': 0.9,
      }});
      map.addLayer({ id: 'fireball-dots', type: 'circle', source: 'fireballs', paint: {
        'circle-radius': ['interpolate',['linear'],['get','energy'], 0,3, 50,7, 500,12],
        'circle-color': '#FFD400', 'circle-opacity': 0.95, 'circle-stroke-width': 1, 'circle-stroke-color': '#3A2A00', 'circle-stroke-opacity': 0.7,
      }});

      // Acoustic contacts (general-audio classifier + operator tags; marine model pending) — magenta sonar ping
      map.addLayer({ id: 'acoustic-glow', type: 'circle', source: 'acoustic-contacts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,6, 6,14], 'circle-color': '#FF00AA', 'circle-opacity': 0.12, 'circle-blur': 0.9,
      }});
      map.addLayer({ id: 'acoustic-dots', type: 'circle', source: 'acoustic-contacts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,3, 6,6, 12,9],
        'circle-color': '#FF00AA', 'circle-opacity': 0.9, 'circle-stroke-width': 1, 'circle-stroke-color': '#1A0010', 'circle-stroke-opacity': 0.6,
      }});
  ...
  // JARVIS Palantir ontology geo objects — split by type, only the toggled types plot
  useEffect(() => {
    if (!mapReady) return;
    const a = activeLayers as any;
    const typeOn: Record<string, boolean> = {
      SpeciesOccurrence: !!a.jarvis_wildlife, Sensor: !!a.jarvis_sensors,
      Measurement: !!a.jarvis_measure, Place: !!a.jarvis_places, Asset: !!a.jarvis_assets,
    };
    const pts = ((data as any).jarvis_geo || []).filter((o: any) => typeOn[o.type] ?? !!a.jarvis_assets);
    setGeo('jarvis-geo', pts.map((o: any) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [o.lng, o.lat] }, properties: { label: o.label, type: o.type } })));
  }, [mapReady, (data as any).jarvis_geo, (activeLayers as any).jarvis_wildlife, (activeLayers as any).jarvis_sensors, (activeLayers as any).jarvis_measure, (activeLayers as any).jarvis_places, (activeLayers as any).jarvis_assets, setGeo]);
  ...
  useEffect(() => {
    if (!mapReady) return;
    setGeo('acoustic-contacts', (activeLayers as any).acoustic_contacts && (data as any).acoustic_contacts ? (data as any).acoustic_contacts.map((c: any) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [c.lng, c.lat] }, properties: { label: c.label, classification: c.classification, confidence: c.confidence } })) : []);
  }, [mapReady, (data as any).acoustic_contacts, (activeLayers as any).acoustic_contacts, setGeo]);

  // NASA CNEOS fireball airbursts (only those with a ground-track lat/lng)
  useEffect(() => {
    if (!mapReady) return;
    setGeo('fireballs', (activeLayers as any).fireballs && (data as any).fireballs ? (data as any).fireballs.filter((f: any) => f.lat != null && f.lng != null).map((f: any) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [f.lng, f.lat] }, properties: { date: f.date, energy: f.energy_kt ?? 0, altitude: f.altitude } })) : []);
  }, [mapReady, (data as any).fireballs, (activeLayers as any).fireballs, setGeo]);
