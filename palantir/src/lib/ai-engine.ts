/**
 * ═══════════════════════════════════════════════════════════════
 *  JARVIS — AI Intelligence Engine
 *  Routed to the local Qwen LLM (Ollama, OpenAI-compatible API).
 *  Default model: qwen2.5:14b on the JARVIS GPU box (127.0.0.1:11434).
 *  Override with QWEN_MODEL / QWEN_BASE_URL env vars (e.g. qwen2.5:32b).
 * ═══════════════════════════════════════════════════════════════
 */

/* ─────────────────────────────────────────────────────────────
   Qwen / Ollama OpenAI-compatible config
   ───────────────────────────────────────────────────────────── */

export const QWEN_MODEL = process.env.QWEN_MODEL || 'qwen2.5:14b';
const QWEN_BASE_URL = (process.env.QWEN_BASE_URL || 'http://127.0.0.1:11434/v1').replace(/\/$/, '');
const QWEN_API_KEY = process.env.QWEN_API_KEY || 'ollama'; // Ollama ignores it; placeholder keeps the OpenAI client happy
const QWEN_TIMEOUT_MS = Number(process.env.QWEN_TIMEOUT_MS || 120_000);

/** Minimal Qwen chat call over the OpenAI-compatible /chat/completions route. */
async function callQwen(systemPrompt: string, userPrompt: string): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Qwen (${QWEN_MODEL}) returned ${res.status}: ${detail.slice(0, 300)}`);
  }

  const data = await res.json();
  const content = data?.choices?.[0]?.message?.content;
  if (typeof content !== 'string' || content.trim().length === 0) {
    throw new Error('Qwen returned an empty completion');
  }
  return content;
}
...
const SYSTEM_PROMPT = `You are JARVIS Intelligence Analyst — a senior, elite intelligence analyst embedded within the JARVIS · PALANTIR Global Intelligence Platform. You operate at the level of a Palantir Forward Deployed Engineer crossed with a CIA PDB (Presidential Daily Brief) analyst.
...
You have access to the live intelligence context of the JARVIS platform. Analyze it with precision.`;

const BRIEFING_PROMPT = `Generate a comprehensive JARVIS Daily Intelligence Briefing based on the current operational data. Structure it as follows:
...
/* ─────────────────────────────────────────────────────────────
   Client Factory (compat shim — Qwen needs no client object)
   ───────────────────────────────────────────────────────────── */
...
/** Kept for call-site compatibility with the previous Gemini engine. */
export function createGeminiClient(_apiKey?: string): QwenClient {
  return { model: QWEN_MODEL };
}

/* ─────────────────────────────────────────────────────────────
   API Key Rotation (compat shim — no external key needed)
   ───────────────────────────────────────────────────────────── */

export function rotateApiKey(keys: string[]): string {
  return keys[0] ?? 'qwen-local';
}
...
Provide your intelligence assessment based on the operational data above and the analyst's query.`;
  ...
  return callQwen(SYSTEM_PROMPT, prompt);
}

/* ─────────────────────────────────────────────────────────────
   Daily Briefing Generation (Qwen)
   ───────────────────────────────────────────────────────────── */
