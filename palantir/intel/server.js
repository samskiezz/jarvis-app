...
async function boot() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    // JARVIS Nexus — announce to the registry + heartbeat (fire-and-forget).
    const NEXUS = process.env.JARVIS_BACKEND_URL || process.env.JARVIS_API_BASE || 'http://127.0.0.1:8001';
    const announce = () => fetch(`${NEXUS}/v1/registry/announce`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: 'jarvis-palantir-intel', name: 'jarvis-palantir-intel', port: Number(PORT),
        role: 'entity-resolver', base_url: `http://127.0.0.1:${PORT}`, health_path: '/health',
        routes: ['/resolve', '/health'], pid: process.pid }),
    }).catch(() => {});
    announce();
    setInterval(() => fetch(`${NEXUS}/v1/registry/heartbeat`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: 'jarvis-palantir-intel', status: 'ok' }),
    }).catch(() => {}), 30000);
  });
}
