...
- [x] F18 JARVIS boot sequence — first load plays a short spoken "all systems online" with the real counts. [2026-06-14] JarvisBootSequence.jsx (pre-built) mounted in App.jsx; fetches /v1/jarvis/system/status + /v1/cinematic/brain on first session load; renders cinematic terminal with real CPU/mem/nodes/synapses lines; click-to-speak sends script to /v1/voice/tts; sessionStorage guards single-run; vite build exit 0.
- [x] F19 Ambient reactor hum toggle — WebAudio loop; on/off in the assistant. [2026-06-14] AmbientReactorHum.jsx (pre-built) mounted in App.jsx; pure WebAudio 60Hz sawtooth + harmonics + bandpass noise + LFO tremolo; ◇ HUM toggle at left:1012; voice trigger via isAmbientQuery/jarvis:ambient-toggle already wired in JarvisBrain; vite build exit 0.
- [x] F20 "Show me" navigation — already in JarvisBrain; extend keyword map to data drill (e.g. "show risks"). [2026-06-14] ShowMeNavigation.jsx SHOW_ME_MAP extended from 10 to 69 entries covering all panels added in F20+; specific entries ordered before broad fallbacks to avoid JarvisBrain handler conflicts; normalized queries verified against full intent-chain; shortcodes (icws, kinv, ctl, etc.) used where natural language would be caught by earlier handlers; vite build exit 0.
- [x] F21 Live clock + uptime (real process uptime from system status). (2026-06-14: LiveClockUptime.jsx mounted in App.jsx; 1s clock tick + 30s poll of /v1/jarvis/system/status for process uptime; bottom-left fixed HUD display; isClockQuery/buildClockScript already wired in JarvisBrain; vite build exit 0)
- [x] F22 Alert toasts — poll /v1/ops alerts → JARVIS announces new criticals (spoken). [2026-06-14] AlertToasts.jsx mounted in App.jsx; polls /v1/alerts?status=open every 20s; severity-sorted toasts top-right; criticals (≥90) spoken via /v1/voice/tts; 12s auto-dismiss; vite build exit 0.
