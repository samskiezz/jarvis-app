...
- [x] F05 Spoken status report — "JARVIS, status" → reads real system+brain numbers aloud (TTS). [2026-09-18] Wired isStatusQuery+buildStatusScript from SpokenStatusReport into JarvisBrain.ask(); status queries now bypass the agent and speak real /v1/jarvis/system/status + /v1/cinematic/brain telemetry via TTS.
