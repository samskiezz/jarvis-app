...
- **Wave 6** (merged to `main`, commit `a9d7269`):
  ...
  - ◐ **P6 Geospatial** #46-51 — `/v1/geo`: objects-with-coords, radius/bbox, geofences (point-in-polygon), layer catalog + GeoJSON, density grid, tracks → **GeoWorkspace** page (offline SVG map). *Honest:* `entities` layer is REAL (ontology coords), `density` DERIVED; `seismic/air/buoys/flight` return honest-empty (`note: source not wired`) until the async fetchers are bound.
