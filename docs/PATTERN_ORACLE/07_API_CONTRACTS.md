...
## 23. `params` REFERENCE MATRIX — every key, by domain, with its handler effect

Authoritative cross-reference of every `params.*` key the live engine reads, which handler consumes it, its default, and what it does. Grounded directly in `_series_from_params`, `classify`, and the `_predict_*` handlers. Unknown keys are accepted and ignored (the request schema is `additionalProperties: true` on `params`).

### 23.1 Routing / shared keys (read by `classify` / `predict`)
| Key | Type | Default | Effect |
|---|---|---|---|
| `domain` | enum | (classified) | Forces the route, bypassing Kimi + regex (`classify` step 2). One of crypto/seismic/trajectory/growth/generic. |
| `target` | string | (parsed) | Subject (ticker, label). Backfilled from the question for crypto. |
| `horizon_hours` | number>0 | (parsed from text, else per-domain) | Overrides any horizon parsed from the question; converted to steps/days/minutes per domain. |

### 23.2 Series keys (read by `_series_from_params`)
| Key | Type | Notes |
|---|---|---|
| `series` | array of number OR `{t,v}` | Primary generic series; `{t}` (ms) enables timestamp-aware sampling-interval inference. |
| `values` | array of number | Alias for `series` (numbers only). |
| `prices` | array of number | Alias for `series` (crypto). |
| precedence | — | `series` → `values` → `prices` (first non-empty wins). |

### 23.3 Crypto keys (`_predict_crypto`)
| Key | Type | Default | Effect |
|---|---|---|---|
| `lookback_days` | int≥1 | 90 | CoinGecko `market_chart` window when fetching (only when no series supplied). |
| (series via 23.2) | — | — | When ≥3 prices present, the network is skipped entirely. |
| Needs | — | — | `<3` positive prices and no resolvable ticker → soft `insufficient_data`. |

### 23.4 Seismic keys (`_predict_seismic`, `_seismic_mags_from_params`)
| Key | Type | Default | Effect |
|---|---|---|---|
| `magnitude` | number | 5.0 | Target magnitude M for `P(>=1 >= M)`. |
| `magnitudes` | array of number | — | Offline magnitude catalog (skips USGS). |
| `catalog` | array of `{mag}`/number | — | Alias for `magnitudes`. |
| `min_magnitude` | number | 2.5 | USGS `minmagnitude` when fetching. |
| `catalog_days` | number | 30.0 | Catalog window (denominator of the Poisson rate). |
| `latitude`/`longitude` | number | — | Region centre (+`radius_km`) for the USGS query. |
| `radius_km` | number | — | `maxradiuskm`. All three of lat/lng/radius required to filter geographically. |
| `omori` | object | — | **Presence switches to the Omori aftershock branch.** |
| `mainshock_K` | number | 100.0 (in branch) | Omori productivity K (also via `omori.K`). |
| `omori_c` | number | 0.05 | Omori c (days). |
| `omori_p` | number | 1.1 | Omori p exponent. |
| `days_since_mainshock` | number | 1.0 | t since mainshock for the aftershock integral. |

### 23.5 Trajectory keys (`_predict_trajectory`)
| Key | Type | Default | Effect |
|---|---|---|---|
| `state_vector` | object | (or top-level params) | `{lat,lng,alt_m,speed_mps,heading_deg,vertical_rate_mps}`; lat/lng/speed/heading required else soft error. |
| `minutes` | number | 10.0 | Great-circle horizon (overridden by `horizon_hours*60`). |
| `semi_major_axis_km` | number | — | **Orbital-period branch** (Kepler III). |
| `a_km` | number | — | Alias for `semi_major_axis_km` (needs "orbit" in the question). |
| `projectile` | object | — | **Presence switches to the ballistic branch.** |
| `speed` | number | 100.0 | Ballistic muzzle speed (m/s); with `angle_deg` triggers ballistic. |
| `angle_deg` | number | 45.0 | Ballistic launch angle. |
| `height0` | number | 0.0 | Ballistic launch height (passed to `projectile_range`). |

### 23.6 Growth / generic keys (`_predict_growth`, `_growth_steps`)
| Key | Type | Default | Effect |
|---|---|---|---|
| (series via 23.2) | — | — | ≥3 points required; chooses exp vs logistic by lower SSE. |
| `horizon_steps` | int≥1 | from horizon/interval, else `n//4` | Explicit projection step count (takes precedence). |
| `unit` | string | null | Display unit echoed into `prediction.unit`. |

### 23.7 Branch-selection precedence (within `_predict_trajectory`/`_predict_seismic`)
1. **Seismic:** `omori`/`mainshock_K` present → Omori branch; else G-R+Poisson on the catalog.
2. **Trajectory:** `semi_major_axis_km` (or `a_km`+"orbit") → orbital; else `projectile`/(`speed`&`angle_deg`) → ballistic; else great-circle from a state vector.

---

## 24. AUTH & CONFIG BEHAVIOUR MATRIX

Exact, code-grounded behaviour of the auth layer (`server/auth.py`) under the config flags (`server/config.py`: `API_KEY`, `REQUIRE_AUTH`, plus `JARVIS_CORS_ORIGINS`). The two dependencies are `optional_bearer` (reads) and `require_bearer` (writes / strict).

### 24.1 `optional_bearer` × `REQUIRE_AUTH` × header
| `REQUIRE_AUTH` | `Authorization` header | Outcome |
|---|---|---|
| false (default) | absent | **Allowed** — returns `None` (public read; local/playable build streams without a key). |
| false | `Bearer <correct>` | Allowed — token validated, returned. |
| false | `Bearer <wrong>` | **401** `invalid token` (a supplied token is always checked). |
| false | malformed (no `Bearer `) | **401** `missing bearer token`. |
| true | absent | **401** `missing bearer token` (behaves like `require_bearer`). |
| true | `Bearer <correct>` | Allowed. |
| true | `Bearer <wrong>` | **401** `invalid token`. |

### 24.2 `require_bearer` (always strict — used by `/v1/predict/backtest`, `/auth/me`, entities)
| `Authorization` header | Outcome |
|---|---|
| absent / malformed | **401** `missing bearer token` (regardless of `REQUIRE_AUTH`). |
| `Bearer <correct>` | Allowed. |
| `Bearer <wrong>` | **401** `invalid token`. |

### 24.3 Error-message fidelity
The `unauthorized` envelope's `message` reproduces `server/auth.py::_check` exactly:
- no/`!Bearer ` header → `"missing bearer token"`.
- token `!= API_KEY` → `"invalid token"`.

Clients should branch on `error.code == "unauthorized"` (HTTP 401), not the message text.

### 24.4 Config flags consumed at the boundary
| Flag (env) | Where | Effect on contracts |
|---|---|---|
| `JARVIS_API_KEY` → `API_KEY` | `auth._check` | The single accepted bearer value. |
| `JARVIS_REQUIRE_AUTH` → `REQUIRE_AUTH` | `optional_bearer` | Flips read endpoints between public and locked (§24.1). |
| `JARVIS_CORS_ORIGINS` | `main.py` CORS | Allowed browser origins; a disallowed origin is blocked by the browser (no special API code). |
| `KIMI_API_KEY` / `KIMI_BASE_URL` / `KIMI_MODEL` | `_kimi_extract` | When key present, NL intent extraction runs and `used_llm` may be `true`; absent → regex fallback, `used_llm=false`. |
| `PREDICT_GPU_URL` (forward) | shadow/remote models | When down → `model_unavailable` (503) on `/v1` model paths; native forecasters unaffected. |

### 24.5 CORS preflight note
- Browser `OPTIONS` preflights are handled by the FastAPI CORS middleware, not the route; they never reach `optional_bearer` and never count against rate-limit buckets.
- Only origins in `JARVIS_CORS_ORIGINS` receive `Access-Control-Allow-Origin`; others fail at the browser. The frontend `kimiClient` attaches the bearer only when a key is configured, which is preflight-safe (simple `Authorization` header).
