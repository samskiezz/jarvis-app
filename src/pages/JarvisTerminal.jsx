  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (Array.isArray(data?.units) || Array.isArray(data?.players) || Array.isArray(data?.agents)) {
          const units = data.units || data.players || data.agents;
          setGameState({ ...data, units });
        }
      } catch {
  ...
  return (
    <div style={{ height:"100%",display:"flex",flexDirection:"column",fontFamily:"Courier New",padding:8,gap:6 }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        ...
        <span style={{ fontSize:7,color:connected?C.neon:C.red }}>{connected?"LIVE":"OFFLINE"} · EVT {events}</span>
      </div>
