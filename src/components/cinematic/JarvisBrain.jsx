...
import { isDkstQuery, buildDkstScript } from "./DatasetKnowledgeScenarioTriple";
...
import { isToexQuery, buildToexScript } from "./TaskOpsEventsExposure";
...
export default function JarvisBrain() {
  ...
  async function ask(q) {
    ...
    if (isSwrinvQuery(q)) {
      ...
      setOpen(true); setThinking(true); setText("");
      const script = await buildSwrinvScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isClieQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:clie-toggle"));
      setOpen(true); setThinking(true); setText("");
      const script = await buildClieScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isSjscliQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:sjscli-toggle"));
      setOpen(true); setThinking(true); setText("");
      const script = await buildSjscliScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isCdoeQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:cdoe-toggle"));
      setOpen(true); setThinking(true); setText("");
      const script = await buildCdoeScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    // F86: contact × investigations correlator — CINVTG opens itself; we speak the brief.
    if (isCinvtgQuery(q)) {
    ...
    if (isToexQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:toex-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    if (isRsaskQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:rsask-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    if (isDgccQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:dgcc-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildDgccScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isRsicQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:rsic-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    if (isJascQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:sjasc-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    if (isSjsiQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:sjsi-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildSjsiScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    if (isGcoeQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:gcoe-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildGcoeScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isInvknowQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:invknow-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildInvknowScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    if (isIkrtQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:ikrt-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    if (isAgcmQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:agcm-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    // F437 — JARVIS Situational Readiness Score
    if (isJsrsQuery(q)) {
    ...
    // F282 — Contact × Graph Community Bridge
    if (isCgibQuery(q)) {
    ...
    if (isKsdmQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:ksdm-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
    ...
    // F447 — Contact × SwarmJob × AIP Skill Intelligence Triple
    if (isCsatQuery(q)) {
    ...
    if (isIgcsnQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:igcsn-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = buildIgcsnScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    ...
    // F246 — Scenario × IntelProfile × Contact Triple
    if (isSipcxQuery(q)) {
    ...
    // F248 — Dataset × Knowledge × Scenario Triple
    if (isDkstQuery(q)) {
    ...
    if (isIkastQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:ikast-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildIkastScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
    if (isTkrbQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:tkrb-toggle", { detail: { query: q } }));
      setOpen(true); setThinking(true); setText("");
      const script = await buildTkrbScript();
      setThinking(false); typeOut(script); speak(script);
      hideT.current = setTimeout(() => setOpen(false), Math.max(9000, script.length * 70));
      return;
    }
