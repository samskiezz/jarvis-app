...
export default function JarvisBrain() {
  ...
  async function ask(q) {
    if (!q || !q.trim()) return;
    // F20: "show me X" pre-router — re-dispatch as normalized query for the target panel.
    if (isShowMeQuery(q)) {
      window.dispatchEvent(new CustomEvent("jarvis:ask", {
        detail: { text: resolveShowMeQuery(q), showMe: true },
      }));
      return;
    }
  ...
  useEffect(() => {
    const onAsk = (e) => {
      ...
      // F20: skip re-dispatched showMe events — target panels handle those directly.
      if (e?.detail?.showMe) return;
