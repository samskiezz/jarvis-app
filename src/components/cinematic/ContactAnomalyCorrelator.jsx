...
const API = '';
const CACR_RE = /\b(contact[._-]?anom|anom[._-]?contact|cacr|flagged[._-]?contacts?|contacts?[._-]?with[._-]?anom|contact[._-]?metric|metric[._-]?contact|which[._-]?contacts?[._-]?have[._-]?anom|contact[._-]?anomaly|anomaly[._-]?contacts?|contact[._-]?z[._-]?score)\b/i;

export function isContactAnomalyQuery(t) {
  return CACR_RE.test(t || '');
}

export async function buildContactAnomalyScript() {
  const [contR, anomR] = await Promise.allSettled([
  ...
  const enriched = correlate(contacts, anomalies);
  const flagged = enriched.filter(c => c._linked).length;
  const clear = enriched.length - flagged;
  return (
    `Contact × Anomaly Correlation: ${contacts.length} contacts, ${anomalies.length} metric anomalies indexed. ` +
    `${flagged} contacts are flagged by active metric anomalies; ${clear} appear clear. ` +
    `Top flagged: ${enriched.filter(c => c._linked).slice(0, 4).map(c => c.name || c.title || c.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(contact, anomaly) {
  ...
  if (!contactToks.size || !anomToks.length) return 0;
  let hits = 0;
  for (const t of anomToks) if (contactToks.has(t)) hits++;
  return hits / Math.max(contactToks.size, anomToks.length);
}

function correlate(contacts, anomalies) {
  return contacts.map(contact => {
    const scored = anomalies
      .map(anom => ({ anom, score: matchScore(contact, anom) }))
      .filter(s => s.score > 0)
...
export default function ContactAnomalyCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:cacr-toggle', onToggle);
    return () => window.removeEventListener('jarvis:cacr-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${contacts.length} contacts and ${anomalies.length} metric anomalies. ${flagged.length} contacts are flagged by active anomalies; ${clear.length} appear clear. Give a 2-sentence contact anomaly brief with the key risk pattern.`,
        }),
      });
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        title="Contact × Anomaly Correlator (CACR)"
        style={{
          position: 'fixed', left: 588960, bottom: 8, zIndex: 222,
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {contact._matches.length > 0 ? (
                        <>
                          ...
                          {contact._matches.map(({ anom, score }, j) => {
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No anomalies matched this contact.</div>
                      )}
                    </div>
                  )}
                </div>
