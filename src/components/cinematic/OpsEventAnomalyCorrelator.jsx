...
const API = '';
const OEAN_RE = /\b(ops[._-]?anom|anom[._-]?ops|oean|ops[._-]?events?[._-]?anom|which[._-]?ops[._-]?events?[._-]?have[._-]?anom|ops[._-]?metric[._-]?anom|anomalous[._-]?ops|ops[._-]?event[._-]?anom[._-]?correlat|ops[._-]?anomaly[._-]?correlat)\b/i;

export function isOeanQuery(t) {
  return OEAN_RE.test(t || '');
}

export async function buildOeanScript() {
  const [evR, anomR] = await Promise.allSettled([
  ...
  const enriched = correlate(events, anomalies);
  const exposed = enriched.filter(e => e._linked).length;
  const clear = enriched.length - exposed;
  return (
    `Ops Events × Anomaly Correlation: ${events.length} ops events, ${anomalies.length} metric anomalies indexed. ` +
    `${exposed} events are backed by active metric anomalies; ${clear} appear clear. ` +
    `Top exposed: ${enriched.filter(e => e._linked).slice(0, 4).map(e => e.event_type || e.title || e.id || '?').join(', ') || 'none'}.`
  );
}
...
function normaliseEvents(raw) {
  ...
  for (const k of ['events', 'items', 'results', 'data', 'ops_events', 'records']) {
...
function matchScore(ev, anomaly) {
  const evToks = new Set([
    ...tokens(ev.event_type),
    ...tokens(ev.title),
  ...
  if (!evToks.size || !anomToks.length) return 0;
  let hits = 0;
  for (const t of anomToks) if (evToks.has(t)) hits++;
  return hits / Math.max(evToks.size, anomToks.length);
}

function correlate(events, anomalies) {
  return events.map(ev => {
    const scored = anomalies
      .map(anom => ({ anom, score: matchScore(ev, anom) }))
      .filter(s => s.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 5);
    return { ...ev, _linked: scored.length > 0, _matches: scored };
  });
}
...
export default function OpsEventAnomalyCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:oean-toggle', onToggle);
    return () => window.removeEventListener('jarvis:oean-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${events.length} ops events and ${anomalies.length} metric anomalies. ${exposed.length} ops events are backed by active anomalies; ${clear.length} appear clear. Give a 2-sentence ops anomaly correlation brief identifying the key risk pattern and which service or event type is most anomalously active.`,
        }),
      });
  ...
  const evLabel = e => e.event_type || e.title || e.id || '?';

  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Ops Events × Anomaly Correlator (OEAN)"
        style={{
          position: 'fixed', left: 716080, bottom: 8, zIndex: 251,
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = ev._linked ? RD : GR;
              return (
                <div
                  key={ev.id || i}
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: '#DCEBF5', fontSize: 11, flex: 1 }}>{evLabel(ev)}</span>
                    {ev.severity && chip(ev.severity, SEVERITY_COLOR[String(ev.severity).toLowerCase()] || AM)}
                    {ev.service && chip(ev.service, PR)}
                    {chip(ev._linked ? 'EXPOSED' : 'CLEAR', statusColor)}
                  ...
                  {isExp && (
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {ev._matches.length > 0 ? (
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No anomalies matched this ops event.</div>
                      )}
                    </div>
                  )}
                </div>
