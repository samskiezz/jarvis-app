...
const API = '';
const DSPC_RE = /\b(dataset[._-]?spec|spec[._-]?dataset|dspc|dataset[._-]?coverage|which[._-]?datasets?[._-]?have[._-]?specs?|dataset[._-]?doc(?:umentation)?|undocumented[._-]?datasets?|spec[._-]?dataset[._-]?coverage)\b/i;

export function isDspcQuery(t) {
  return DSPC_RE.test(t || '');
}

export async function buildDspcScript() {
  const [dsR, spR] = await Promise.allSettled([
  ...
  const enriched = correlate(datasets, specs);
  const documented = enriched.filter(d => d._linked).length;
  const undocumented = enriched.length - documented;
  return (
    `Dataset × Spec Coverage: ${datasets.length} datasets, ${specs.length} specs indexed. ` +
    `${documented} datasets have spec backing; ${undocumented} are undocumented. ` +
    `Top undocumented: ${enriched.filter(d => !d._linked).slice(0, 4).map(d => d.name || d.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(dataset, spec) {
  const dToks = new Set([
    ...tokens(dataset.name),
  ...
  if (!dToks.size || !sToks.length) return 0;
  let hits = 0;
  for (const t of sToks) if (dToks.has(t)) hits++;
  return hits / Math.max(dToks.size, sToks.length);
}

function correlate(datasets, specs) {
  return datasets.map(dataset => {
    const scored = specs
      .map(spec => ({ spec, score: matchScore(dataset, spec) }))
      .filter(s => s.score > 0)
...
const STATUS_COLOR = { approved: GR, draft: AM, review: CY };
...
export default function DatasetSpecCoverage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:dspc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:dspc-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${datasets.length} datasets and ${specs.length} implementation specs. ${documented.length} datasets have spec backing; ${undocumented.length} have no spec coverage. Give a 2-sentence dataset-spec coverage brief identifying the documentation gap risk and which dataset kinds most need spec attention.`,
        }),
      });
  ...
  const datasetLabel = d => d.name || d.id || '?';

  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Dataset × Spec Coverage (DSPC)"
        style={{
          position: 'fixed', left: 725200, bottom: 8, zIndex: 253,
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {dataset.kind && chip(dataset.kind, CY)}
                    {dataset.source && chip(dataset.source, PR)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {dataset._matches.length > 0 ? (
                        <>
                          ...
                          {dataset._matches.map(({ spec, score }, j) => {
                            const stColor = STATUS_COLOR[String(spec.status || '').toLowerCase()] || AM;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {spec.status && chip(spec.status, stColor)}
                                {spec.kind && chip(spec.kind, PR)}
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No specs matched this dataset.</div>
                      )}
                    </div>
                  )}
                </div>
