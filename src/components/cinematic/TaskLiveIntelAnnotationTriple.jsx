...
const TLGANN_RE = /\b(tlgann|task\s+live\s+annotation|task\s+world\s+annotation|task\s+annotation\s+live|wired\s+task|task\s+graph\s+annotation\s+live|task\s+live\s+intel\s+annotation|task\s+annotation\s+world|untracked\s+task\s+live|task\s+live\s+world\s+annotation)\b/i;

export function isTlgannQuery(t) {
  return TLGANN_RE.test(t || '');
}

export async function buildTlgannScript() {
  try {
    const [taskRes, liRes, anRes] = await Promise.all([
    ...
    const classified = tasks.map(t => classifyTask(t, live, anns));
    const fullyWired = classified.filter(t => t.state === 'FULLY_WIRED').length;
    const worldTriggered = classified.filter(t => t.state === 'WORLD_TRIGGERED').length;
    const graphAnnotated = classified.filter(t => t.state === 'GRAPH_ANNOTATED').length;
    const untracked = classified.filter(t => t.state === 'UNTRACKED').length;
    return `TLGANN task-live-annotation coverage: ${tasks.length} tasks — ${fullyWired} fully wired (live intel+annotation), ${worldTriggered} world-triggered only, ${graphAnnotated} graph-annotated only, ${untracked} untracked (neither). ${untracked > 0 ? `${untracked} tasks have no live world event or graph annotation linkage.` : 'All tasks have at least one active linkage.'}`;
  } catch {
    return 'TLGANN data unavailable — check /entities/Task, /functions/getLiveIntel, and /v1/graph/annotations endpoints.';
  }
}
...
function normaliseTasks(raw) {
  ...
  return arr.map((t, i) => ({
    id: t.id || t._id || t.uid || `task-${i}`,
    name: t.name || t.title || t.mission || `Task ${i + 1}`,
    description: t.description || t.summary || t.objective || t.notes || '',
...
function classifyTask(task, liveEvents, annotations) {
  const tToks = tok([task.name, task.description, task.category, task.priority, task.tags].join(' '));

  const matchedLive = liveEvents.filter(e => {
    const score = matchScore(tToks, [e.title, e.type, e.detail].join(' '));
    return score >= THRESHOLD;
  });

  const matchedAnns = annotations.filter(a => {
    const score = matchScore(tToks, [a.label, a.entity, a.notes, a.tags].join(' '));
    return score >= THRESHOLD;
  });
  ...
  return { ...task, state, matchedLive, matchedAnns };
}
...
const STATE_META = {
  ...
  UNTRACKED:       { label: 'UNTRACKED',         color: '#6b7280', bg: 'rgba(107,114,128,0.13)' },
};

const FILTERS = ['ALL', 'FULLY_WIRED', 'WORLD_TRIGGERED', 'GRAPH_ANNOTATED', 'UNTRACKED'];

export default function TaskLiveIntelAnnotationTriple() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:tlgann-toggle', onToggle);
    return () => window.removeEventListener('jarvis:tlgann-toggle', onToggle);
  }, []);
  ...
  useEffect(() => {
    const onVoice = e => {
      if (TLGANN_RE.test(e.detail?.query || '')) setOpen(true);
    };
  ...
  const assess = useCallback(async () => {
    ...
    setAssessment('');
    try {
      const untracked = classified.filter(t => t.state === 'UNTRACKED').map(t => t.name).slice(0, 5).join(', ');
      const wired = classified.filter(t => t.state === 'FULLY_WIRED').map(t => t.name).slice(0, 3).join(', ');
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: `Assess TLGANN task live-world-annotation coverage. ${tasks.length} tasks. Fully wired (live intel+annotation): ${wired || 'none'}. Untracked (no live or annotation linkage): ${untracked || 'none'}. In 2 sentences, identify the largest coverage gap and recommend the highest-priority intelligence action.`,
        }),
      });
  ...
  }, [classified, tasks.length]);

  if (!open) return null;

  const counts = {
    FULLY_WIRED:     classified.filter(t => t.state === 'FULLY_WIRED').length,
    WORLD_TRIGGERED: classified.filter(t => t.state === 'WORLD_TRIGGERED').length,
    GRAPH_ANNOTATED: classified.filter(t => t.state === 'GRAPH_ANNOTATED').length,
    UNTRACKED:       classified.filter(t => t.state === 'UNTRACKED').length,
  };
  ...
  return (
    <div style={{
      position: 'fixed', left: 834160, bottom: 8, zIndex: 518,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ color: '#22d3ee', fontWeight: 700, letterSpacing: 1 }}>◈ TLGANN</span>
          <span style={{ color: '#64748b', fontSize: 10 }}>TASK × LIVE INTEL × ANNOTATION</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 4, padding: '8px 12px', flexShrink: 0 }}>
        {[
          { label: 'TASKS',        value: tasks.length,           color: '#22d3ee' },
      ...
      <div style={{ padding: '0 12px 8px', flexShrink: 0 }}>
        <div style={{ height: 6, borderRadius: 3, background: '#1e293b', overflow: 'hidden', display: 'flex' }}>
          {(['FULLY_WIRED','WORLD_TRIGGERED','GRAPH_ANNOTATED','UNTRACKED']).map(state => (
            <div key={state} style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px 8px' }}>
        {visible.length === 0 && !loading && (
          <div style={{ color: '#475569', textAlign: 'center', padding: 20 }}>No tasks match</div>
        )}
        {visible.map(item => {
          ...
          return (
            <div key={item.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px' }}>
                ...
                {item.priority && (
                  <span style={{ color: '#475569', fontSize: 9, whiteSpace: 'nowrap' }}>{item.priority}</span>
                )}
