...
const API = '';
const IPRSC_RE = /\b(intel[._-]?risk|risk[._-]?intel[._-]?profile|iprsc|intel[._-]?flag|intel[._-]?with[._-]?risk|which[._-]?intel[._-]?(?:profiles?)?[._-]?have[._-]?risk|intel[._-]?profile[._-]?risk|intel[._-]?risk[._-]?coverage|intel[._-]?risk[._-]?signal|risky[._-]?intel)\b/i;

export function isIprscQuery(t) {
  return IPRSC_RE.test(t || '');
}

export async function buildIprscScript() {
  const [intelR, riskR] = await Promise.allSettled([
    fetch(`${API}/entities/IntelProfile`).then(r => r.json()),
    fetch(`${API}/entities/RiskSignal`).then(r => r.json()),
  ]);
  const profiles = normaliseArray(intelR.status === 'fulfilled' ? intelR.value : []);
  const risks = normaliseArray(riskR.status === 'fulfilled' ? riskR.value : []);
  const enriched = correlate(profiles, risks);
  const flagged = enriched.filter(p => p._linked).length;
  const clear = enriched.length - flagged;
  return (
    `Intel Profile × Risk Signal Correlation: ${profiles.length} intel profiles, ${risks.length} risk signals indexed. ` +
    `${flagged} profiles are exposed to active risk signals; ${clear} appear clear. ` +
...
function normaliseArray(raw) {
  ...
  for (const k of ['items', 'results', 'data', 'profiles', 'risks', 'signals', 'entities', 'records']) {
...
function matchScore(profile, risk) {
  ...
  if (!profToks.size || !riskToks.length) return 0;
  let hits = 0;
  for (const t of riskToks) if (profToks.has(t)) hits++;
  return hits / Math.max(profToks.size, riskToks.length);
}

function correlate(profiles, risks) {
  return profiles.map(prof => {
    const scored = risks
      .map(risk => ({ risk, score: matchScore(prof, risk) }))
      .filter(s => s.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 5);
    return { ...prof, _linked: scored.length > 0, _matches: scored };
  });
}
...
export default function IntelProfileRiskCorrelator() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProfiles(normaliseArray(intelR.status === 'fulfilled' ? intelR.value : []));
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:iprsc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:iprsc-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${profiles.length} intel profiles and ${risks.length} active risk signals. ${flagged.length} profiles are exposed to active risk signals; ${clear.length} appear clear. Give a 2-sentence intel-risk brief with the key threat pattern and recommended action.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Intel Profile × Risk Signal Correlator (IPRSC)"
        style={{
          position: 'fixed', left: 607200, bottom: 8, zIndex: 226,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2, fontWeight: 700, textShadow: `0 0 12px ${CY}` }}>
              ◈ INTEL PROFILE × RISK SIGNAL CORRELATOR
            </span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = prof._linked ? RD : GR;
              return (
                <div
                  key={prof.id || i}
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: '#DCEBF5', fontSize: 11, flex: 1 }}>{label(prof)}</span>
                    {prof.category && chip(prof.category, '#A78BFA')}
                    {prof.nationality && chip(prof.nationality, '#6E8AA0')}
                    {chip(prof._linked ? 'FLAGGED' : 'CLEAR', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {prof._matches.length > 0 ? (
                        <>
                          ...
                          {prof._matches.map(({ risk, score }, j) => {
                            const sevColor = SEVERITY_COLOR[String(risk.severity || '').toLowerCase()] || AM;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {risk.severity && chip(risk.severity, sevColor)}
                                {risk.category && chip(risk.category, '#6E8AA0')}
                                <span style={{ color: '#DCEBF5', fontSize: 10, flex: 1 }}>
                                  {risk.title || risk.name || risk.id || '?'}
                                </span>
                                {scorebar(score, RD)}
                              </div>
                            );
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No risk signals matched this intel profile.</div>
                      )}
                    </div>
                  )}
                </div>
