/**
 * AcousticSwarmJobNexus — F671
 * "JARVIS, acswrm / acoustic swarm / swarm acoustic / swarm contact /
 *  acoustic job / sensor swarm / acoustic swarm job / sound automation"
 * Cross-references /v1/acoustic/contacts against /entities/SwarmJob.
 * ASSIGNED contacts (≥1 swarm job keyword-match) vs UNASSIGNED (no swarm backing).
 * Coverage % tile; ALL/ASSIGNED/UNASSIGNED filter tabs + search; click-to-expand matched jobs.
...
const BTN_LEFT = 127_740;
const Z_INDEX  = 207;

const ACSWRM_RE =
  /\bacswrm\b|\bacoustic.?swarm\b|\bswarm.?acoustic\b|\bswarm.?contact\b|\bacoustic.?job\b|\bsensor.?swarm\b|\bacoustic.?swarm.?job\b|\bsound.?automation\b/i;

export function isAcswrmQuery(text) {
  return ACSWRM_RE.test(text || "");
}
...
function normaliseJobs(data) {
  ...
  return arr.map((j, i) => ({
    id:          j.id          || `job-${i}`,
    title:       j.title       || j.name        || j.job_name || `Job ${i + 1}`,
    status:      (j.status     || j.state       || "PENDING").toString().toUpperCase(),
    progress:    j.progress    != null ? j.progress : null,
    description: j.description || j.body        || j.notes   || "",
  }));
}
...
function crossRef(contacts, jobs) {
  return contacts.map((contact) => {
    ...
    return { ...contact, assigned: matches.length > 0, jobs: matches };
  });
}

export async function buildAcswrmScript() {
  try {
    const base = apiBase();
    const hdr  = { Authorization: `Bearer ${API_KEY}` };
    const [acRes, jobRes] = await Promise.all([
      fetch(`${base}/v1/acoustic/contacts?limit=200`, { headers: hdr }),
      fetch(`${base}/entities/SwarmJob`,              { headers: hdr }),
    ]);
    const [acData, jobData] = await Promise.all([acRes.json(), jobRes.json()]);
    ...
    const pct = rows.length ? Math.round((assigned / rows.length) * 100) : 0;
    if (!rows.length) return "No acoustic contacts found in the system, sir.";
    const topAssigned = rows
      .filter((r) => r.assigned)
      .slice(0, 2)
      .map((r) => `${r.label} → ${r.jobs[0]?.title || "?"}`)
      .join("; ");
    const brief = `Acoustic × SwarmJob cross-reference: ${assigned} of ${rows.length} contacts matched to active swarm jobs (${pct}%) — ${topAssigned || "none"}.`;
    const aiRes = await fetch(`${base}/v1/jarvis/agent/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${API_KEY}` },
      body: JSON.stringify({
        message: `Acoustic contacts cross-referenced against ${jobs.length} swarm jobs: ${assigned} assigned / ${unassigned} unassigned. Top matches: ${topAssigned || "none"}. Provide a 2-sentence sensor-swarm automation assessment.`,
      }),
    });
    const aiData = await aiRes.json();
    return aiData?.response || aiData?.message || brief;
  } catch (e) {
    return `Acoustic swarm cross-reference error: ${e.message}`;
  }
}
...
function statusColor(status) {
  ...
  if (s === "RUNNING")  return CY;
  if (s === "DONE" || s === "COMPLETE" || s === "COMPLETED") return GRN;
  return AMB;
}

export default function AcousticSwarmJobNexus() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const contacts = normaliseContacts(acData);
      const jobs     = normaliseJobs(jobData);
      setRows(crossRef(contacts, jobs));
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:acswrm-toggle", handler);
    return () => window.removeEventListener("jarvis:acswrm-toggle", handler);
  }, []);
  ...
  const assess = async () => {
    ...
    setBrief("");
    try {
      const txt = await buildAcswrmScript();
      setBrief(txt);
  ...
  const visible = rows.filter((r) => {
    if (tab === "ASSIGNED"   && !r.assigned) return false;
    ...
    if (search) {
      ...
      return (
        ...
        r.jobs.some((j) => j.title.toLowerCase().includes(q))
      );
    }
    return true;
  });
  ...
  const btnStyle = {
    ...
    left: BTN_LEFT,
    zIndex: Z_INDEX,
    background: assigned > 0 ? `${AMB}22` : "#0A1628CC",
    border: `1px solid ${assigned > 0 ? AMB : CY}66`,
    borderRadius: 6,
    color: assigned > 0 ? AMB : CY,
    padding: "3px 7px",
    cursor: "pointer",
    fontSize: 9,
    letterSpacing: 1,
      ...
      {open && (
        <div style={panelStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ color: CY, fontSize: 11, letterSpacing: 1 }}>◈ ACOUSTIC × SWARM NEXUS</span>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            {[
              ...
              { label: "ASSIGNED",   val: assigned,    color: AMB },
              { label: "UNASSIGNED", val: unassigned,  color: GRN },
          ...
          <input
            ...
            placeholder="search contacts or swarm jobs…"
            style={{
              background: "#0A1628",
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {visible.map((row) => (
              <div
                ...
                style={{
                  marginBottom: 4,
                  background: row.assigned ? `${AMB}0A` : "#0A162855",
                  border: `1px solid ${row.assigned ? AMB : DIM}33`,
                  borderRadius: 5,
                  overflow: "hidden",
                }}
              >
                ...
                >
                  <span style={{
                    fontSize: 8,
                    background: `${row.assigned ? AMB : GRN}22`,
                    border: `1px solid ${row.assigned ? AMB : GRN}55`,
                    color: row.assigned ? AMB : GRN,
                    borderRadius: 3,
                    padding: "1px 4px",
                    minWidth: 68,
                    textAlign: "center",
                  ...
                  {row.assigned && (
                    <span style={{ color: AMB, fontSize: 8 }}>{row.jobs.length} job{row.jobs.length !== 1 ? "s" : ""}</span>
                  )}
                </div>
                {expanded === row.id && (
                    ...
                    {row.jobs.length > 0 ? (
                      row.jobs.slice(0, 5).map((job) => (
                        ...
                        >
                          ...
                          {job.progress != null && (
                            <span style={{ color: DIM, fontSize: 9 }}>{Math.round(job.progress * 100)}%</span>
                          )}
                          <span style={{ color: DIM, fontSize: 9 }}>hits:{job.hits}</span>
                        </div>
                      ))
                    ) : (
                      <div style={{ color: DIM, fontSize: 10 }}>No swarm job correlation — contact is unassigned.</div>
                    )}
                  </div>
                )}
              </div>
