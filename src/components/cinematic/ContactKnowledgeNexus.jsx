/**
 * ContactKnowledgeNexus — F511
 * "JARVIS, contact knowledge / who is documented / cknow / documented contacts / contact articles / knowledge contacts"
 * Cross-references /entities/Contact + /knowledge/.
 * Finds DOCUMENTED contacts (≥1 knowledge article keyword-matches the contact name/tags)
 * vs UNDOCUMENTED contacts (intelligence blind spots with no knowledge support).
...
const BTN_LEFT = 31_560;
const Z_INDEX  = 95;

const CKNOW_RE =
  /\bcknow\b|\bcontact.?knowledge\b|\bknowledge.?contact\b|\bwho.?is.?documented\b|\bdocumented.?contact\b|\bcontact.?article\b|\bknowledge.?about.?contact\b|\bintel.?on.?contact\b|\bcontact.?intel.?gap\b|\bundocumented.?contact\b/i;

export function isCknowQuery(text) {
  return CKNOW_RE.test(text || "");
}
...
function normaliseContacts(data) {
  ...
  return raw.map((c, i) => ({
    ...
    org:  c.organization || c.org || c.company || null,
...
function crossRef(contacts, articles) {
  return contacts.map((ct) => {
    const haystack = `${ct.name} ${ct.role || ""} ${ct.org || ""} ${ct.tags}`;
...
// ─── buildCknowScript (for JarvisBrain) ──────────────────────────────────────

export async function buildCknowScript() {
  try {
    const base = apiBase();
    const hdr = { Authorization: `Bearer ${API_KEY}` };
    const [ctRes, artRes] = await Promise.all([
    ...
    const crossed   = crossRef(contacts, articles);

    const total      = crossed.length;
    const documented = crossed.filter((c) => c.documented).length;
    ...
    const brief =
      `${coverage}% of ${total} contacts have knowledge documentation. ` +
      `${documented} DOCUMENTED, ${undoc} UNDOCUMENTED.` +
      (topNames ? ` Top documented contacts: ${topNames}.` : "");

    const agentRes = await fetch(`${base}/v1/jarvis/agent/chat`, {
      method: "POST",
      headers: { ...hdr, "Content-Type": "application/json" },
      body: JSON.stringify({
        message: `Contact × Knowledge Coverage: ${brief} Provide a 2-sentence intelligence assessment.`,
      }),
    });
    ...
    return `Contact × Knowledge Nexus unavailable: ${err.message}`;
  }
}
...
export default function ContactKnowledgeNexus() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:cknow-toggle", onToggle);
    return () => window.removeEventListener("jarvis:cknow-toggle", onToggle);
  }, [load]);
  ...
  const assess = useCallback(async () => {
    ...
    try {
      ...
      const undoc      = total - documented;
      const coverage   = total > 0 ? Math.round((documented / total) * 100) : 0;
      const prompt = `Contact × Knowledge Coverage: ${coverage}% coverage (${documented}/${total} documented, ${undoc} undocumented intel blind spots). Assess in 2 sentences.`;
  ...
  const visible = crossed.filter((ct) => {
    if (tab === "DOCUMENTED"   && !ct.documented) return false;
    if (tab === "UNDOCUMENTED" &&  ct.documented) return false;
    if (query) {
      const q = query.toLowerCase();
      if (
        !ct.name.toLowerCase().includes(q) &&
        !(ct.role || "").toLowerCase().includes(q) &&
        !(ct.org  || "").toLowerCase().includes(q)
      )
        return false;
    }
    return true;
  ...
  const total      = crossed.length;
  const nDoc       = crossed.filter((c) => c.documented).length;
              ...
              <div key={ct.id}>
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${ct.documented ? GRN + "44" : DIM + "22"}`,
                  }}
                >
                  <span
                    style={{
                      fontSize: 9,
                      color: ct.documented ? GRN : DIM,
                      minWidth: 78,
                    }}
                  >
                    {ct.documented ? "DOCUMENTED" : "UNDOCUMENTED"}
                  </span>
                  <span
                    style={{
                      flex: 1,
                  ...
                  {ct.documented && (
                    <span style={{ fontSize: 8, color: GRN }}>
                      ⬡ {ct.matches.length} art
                    </span>
                  )}
                </div>

                {/* Expanded matched articles */}
                {expanded === ct.id && ct.documented && (
                ...
                {expanded === ct.id && !ct.documented && (
                  ...
                  >
                    No knowledge articles reference this contact.
                    {ct.org && <div style={{ marginTop: 2 }}>Org: {ct.org}</div>}
                  </div>
                )}
              </div>
            ))
