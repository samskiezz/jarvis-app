...
const API = '';
const CACL_RE = /\b(contact[._-]?alert|alert[._-]?contact|cacl|contacts?[._-]?with[._-]?alert|flagged[._-]?contact|alert[._-]?contact[._-]?correlat|which[._-]?contacts?[._-]?have[._-]?alert)\b/i;

export function isCaclQuery(t) {
  return CACL_RE.test(t || '');
}

export async function buildCaclScript() {
  const [conR, alR] = await Promise.allSettled([
  ...
  const enriched = correlate(contacts, alerts);
  const flagged = enriched.filter(c => c._linked).length;
  const clear = enriched.length - flagged;
  return (
    `Contact × Alert Correlation: ${contacts.length} contacts, ${alerts.length} active alerts indexed. ` +
    `${flagged} contacts are matched to active alerts; ${clear} appear clear. ` +
    `Top flagged: ${enriched.filter(c => c._linked).slice(0, 4).map(c => c.name || c.email || c.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(contact, alert) {
  const cToks = new Set([
    ...tokens(contact.name),
...
function correlate(contacts, alerts) {
  return contacts.map(contact => {
    const scored = alerts
      .map(alert => ({ alert, score: matchScore(contact, alert) }))
      .filter(s => s.score > 0)
...
export default function ContactAlertCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:cacl-toggle', onToggle);
    return () => window.removeEventListener('jarvis:cacl-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(contacts, alerts);
  const flagged = enriched.filter(c => c._linked);
  const clear = enriched.filter(c => !c._linked);
  const badgeCount = flagged.length;
  const badgeColor = badgeCount > 0 ? RD : GR;

  const filtered = enriched
    .filter(c => tab === 'ALL' || (tab === 'FLAGGED' ? c._linked : !c._linked))
    .filter(c => {
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${contacts.length} contacts and ${alerts.length} active alerts. ${flagged.length} contacts are matched to active alerts; ${clear.length} appear clear. Give a 2-sentence contact alert correlation brief identifying the key risk pattern and which contacts or categories are most alert-correlated.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Contact × Alert Correlator (CACL)"
        style={{
          position: 'fixed', left: 720640, bottom: 8, zIndex: 252,
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              ...
              { label: 'ALERTS', val: alerts.length, col: AM },
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = contact._linked ? RD : GR;
              return (
                <div
                  key={contact.id || i}
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: '#DCEBF5', fontSize: 11, flex: 1 }}>{contactLabel(contact)}</span>
                    {contact.company && chip(contact.company, PR)}
                    {contact.category && chip(contact.category, AM)}
                    {chip(contact._linked ? 'FLAGGED' : 'CLEAR', statusColor)}
                  ...
                  {isExp && (
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {contact._matches.length > 0 ? (
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No alerts matched this contact.</div>
                      )}
                    </div>
                  )}
                </div>
