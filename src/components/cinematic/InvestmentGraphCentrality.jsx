...
const API = '';
const IVGC_RE = /\b(invest(?:ment)?[._-]?central(?:ity)?|ivgc|central[._-]?invest(?:ment)?s?|portfolio[._-]?central(?:ity)?|invest(?:ment)?[._-]?network[._-]?rank|which[._-]?invest(?:ment)?s?[._-]?are[._-]?central)\b/i;

export function isIvgcQuery(t) {
  return IVGC_RE.test(t || '');
}

export async function buildIvgcScript() {
  ...
  const enriched = correlate(investments, nodes);
  const highCentrality = enriched.filter(inv => inv._matches.length > 0).length;
  const peripheral = enriched.length - highCentrality;
  const topNames = enriched
    .filter(inv => inv._matches.length > 0)
    .slice(0, 4)
    .map(inv => inv.name || inv.title || inv.ticker || inv.id || '?')
    .join(', ') || 'none';
  return (
    `Investment × Graph Centrality: ${investments.length} investments, ${nodes.length} central nodes indexed. ` +
    `${highCentrality} investments are HIGH-CENTRALITY (graph-linked); ${peripheral} are PERIPHERAL. ` +
    `Top graph-linked investments: ${topNames}.`
  );
}
...
function normaliseInvestments(raw) {
  ...
  for (const k of ['items', 'investments', 'results', 'data', 'records', 'entities']) {
...
function matchScore(investment, node) {
  const iToks = new Set([
  ...
  if (!iToks.size || !nToks.length) return 0;
  let hits = 0;
  for (const t of nToks) if (iToks.has(t)) hits++;
  return hits / Math.max(iToks.size, nToks.length);
}

function correlate(investments, nodes) {
  return investments.map(inv => {
    const scored = nodes
      .map(n => ({ n, score: matchScore(inv, n) }))
      .filter(x => x.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 5);
    return { ...inv, _matches: scored };
  });
}
...
export default function InvestmentGraphCentrality() {
  ...
  useEffect(() => {
    const h = () => setOpen(v => !v);
    window.addEventListener('jarvis:ivgc-toggle', h);
    return () => window.removeEventListener('jarvis:ivgc-toggle', h);
  }, []);
  ...
  const assess = async () => {
    setAssessing(true);
    setAssessment('');
    const high = enriched.filter(inv => inv._matches.length > 0);
    const prompt =
      `Investment × Graph Centrality: ${investments.length} investments, ${nodes.length} central nodes. ` +
      `${high.length} investments are HIGH-CENTRALITY (graph-linked); ${enriched.length - high.length} are PERIPHERAL. ` +
      `Top linked: ${high.slice(0, 5).map(inv => inv.name || inv.title || inv.ticker || inv.id || '?').join(', ') || 'none'}. ` +
      `Give a 2-sentence network investment brief.`;
    try {
  ...
  const highCount = enriched.filter(inv => inv._matches.length > 0).length;
  const badge = highCount > 0 ? '#22c55e' : '#64748b';

  const visible = enriched.filter(inv => {
    const label = (inv.name || inv.title || inv.ticker || inv.id || '').toLowerCase();
    if (search && !label.includes(search.toLowerCase())) return false;
    if (tab === 'HIGH-CENTRALITY') return inv._matches.length > 0;
    if (tab === 'PERIPHERAL') return inv._matches.length === 0;
    return true;
  });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(v => !v)}
        title="Investment × Graph Centrality"
        style={{
          position: 'fixed',
          left: 575280,
          bottom: 8,
      ...
      {open && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px 10px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
            <span style={{ fontWeight: 700, fontSize: 13, letterSpacing: 1, color: ACCENT }}>◈ INVESTMENT × GRAPH CENTRALITY</span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={assess}
                disabled={assessing}
                style={{ background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.3)', borderRadius: 6, color: ACCENT, padding: '3px 10px', fontSize: 11, cursor: 'pointer' }}
              >
                {assessing ? '...' : '▶ ASSESS'}
              </button>
          ...
          <div style={{ display: 'flex', gap: 8, padding: '12px 16px 8px', flexWrap: 'wrap' }}>
            {[
              { label: 'INVESTMENTS', val: investments.length, color: '#34d399' },
              { label: 'CENTRAL NODES', val: nodes.length, color: '#60a5fa' },
              { label: 'HIGH-CENTRALITY', val: highCount, color: highCount > 0 ? '#22c55e' : '#64748b' },
          ...
          {assessment && (
            <div style={{ margin: '0 16px 10px', padding: '10px 12px', background: 'rgba(52,211,153,0.07)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 8, fontSize: 12, color: '#6ee7b7', lineHeight: 1.5 }}>
              {assessment}
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 16px 8px', flexWrap: 'wrap' }}>
            {['ALL', 'HIGH-CENTRALITY', 'PERIPHERAL'].map(t => (
              <button
                ...
                onClick={() => setTab(t)}
                style={{
                  background: tab === t ? 'rgba(52,211,153,0.15)' : 'rgba(255,255,255,0.05)',
                  border: `1px solid ${tab === t ? 'rgba(52,211,153,0.4)' : 'rgba(255,255,255,0.1)'}`,
                  borderRadius: 6,
          ...
          <div>
            {visible.map((inv, i) => {
              ...
              const label = inv.name || inv.title || inv.ticker || `Investment ${id}`;
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{
                      ...PILL,
                      background: inv._matches.length > 0 ? 'rgba(34,197,94,0.12)' : 'rgba(100,116,139,0.12)',
                      color: inv._matches.length > 0 ? '#22c55e' : '#64748b',
                      border: `1px solid ${inv._matches.length > 0 ? 'rgba(34,197,94,0.3)' : 'rgba(100,116,139,0.25)'}`,
                    }}>
                      {inv._matches.length > 0 ? 'HIGH-CENTRALITY' : 'PERIPHERAL'}
                    </span>
                    {sector && (
                      <span style={{ ...PILL, background: 'rgba(52,211,153,0.10)', color: '#34d399', border: '1px solid rgba(52,211,153,0.25)' }}>
                        {sector}
                      </span>
                    )}
                    {ticker && (
                    ...
                    {inv._matches.length > 0 && (
                      <span style={{ color: '#64748b', fontSize: 10 }}>{inv._matches.length} node{inv._matches.length !== 1 ? 's' : ''}</span>
                    )}
                  ...
                  {isExp && (
                    <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                      {inv._matches.length > 0 ? (
                        ...
                        <div style={{ color: '#64748b', fontSize: 11 }}>No central graph nodes matched for this investment.</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div style={{ padding: '8px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', color: '#475569', fontSize: 10 }}>
            {visible.length} of {enriched.length} investments · {nodes.length} central nodes indexed · auto-refresh 90s
          </div>
        </div>
      )}
    </>
