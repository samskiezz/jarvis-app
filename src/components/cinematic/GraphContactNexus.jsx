/**
 * GraphContactNexus — F585
 * "JARVIS, graph contact / grcnt / which nodes have contacts / contact nodes / contact network / node contacts / graph people"
 * Cross-references /v1/graph/centrality (top influential nodes) + /entities/Contact.
 * Finds ASSOCIATED nodes (≥1 contact keyword-matches) vs UNLINKED (no contact backing).
 * Coverage % tile; ALL/ASSOCIATED/UNLINKED filter tabs + search; click-to-expand matched contacts.
 * ▶ ASSESS → /v1/jarvis/agent/chat 2-sentence graph-contact intelligence brief + TTS.
...
const BTN_LEFT = 72_840;
const Z_INDEX  = 143;

const GRCNT_RE =
  /\bgrcnt\b|\bgraph.?contact\b|\bcontact.?graph\b|\bwhich.?nodes?.?have.?contact\b|\bcontact.?nodes?\b|\bcontact.?network\b|\bnode.?contact\b|\bgraph.?people\b|\bpeople.?in.?graph\b|\bnode.?person\b/i;

export function isGrcntQuery(text) {
  return GRCNT_RE.test(text || "");
}
...
function normaliseContacts(raw) {
  ...
  return arr.map((c, i) => ({
    ...
    role:         (c.role       || c.title      || c.position || "").toString(),
    organisation: (c.organisation || c.org     || c.company  || "").toString(),
    tags:         Array.isArray(c.tags) ? c.tags.join(" ") : (c.tags || ""),
    email:        (c.email      || "").toString(),
  }));
}

function crossRef(nodes, contacts) {
  return nodes.map((node) => {
    const haystack = `${node.label} ${node.description} ${node.tags} ${node.kind}`;
    const matches = contacts
      .map((ct) => ({
        ct,
        hits: overlap(haystack, `${ct.name} ${ct.role} ${ct.organisation} ${ct.tags}`),
      }))
      .filter(({ hits }) => hits > 0)
    ...
    return {
      ...
      matches: matches.map(({ ct, hits }) => ({ ...ct, hits })),
    };
  });
}

// ─── buildGrcntScript (for JarvisBrain) ──────────────────────────────────────

export async function buildGrcntScript() {
  try {
    const base = apiBase();
    const hdr = { Authorization: `Bearer ${API_KEY}` };
    const [nodeRes, ctRes] = await Promise.all([
    ...
    const crossed  = crossRef(nodes, contacts);

    const total      = crossed.length;
    const associated = crossed.filter((n) => n.associated).length;
    const unlinked   = total - associated;
    const coverage   = total > 0 ? Math.round((associated / total) * 100) : 0;
    const topUnlinked = crossed
      .filter((n) => !n.associated)
      .slice(0, 2)
      .map((n) => n.label)
      .join(", ");

    const brief =
      `${coverage}% of ${total} top graph nodes are associated with contacts. ` +
      `${associated} ASSOCIATED, ${unlinked} UNLINKED.` +
      (topUnlinked ? ` Top unlinked nodes: ${topUnlinked}.` : "");

    const agentRes = await fetch(`${base}/v1/jarvis/agent/chat`, {
      method: "POST",
      headers: { ...hdr, "Content-Type": "application/json" },
      body: JSON.stringify({
        message: `Graph × Contact Nexus: ${brief} Provide a 2-sentence graph-contact intelligence assessment.`,
      }),
    });
    ...
    return `Graph × Contact Nexus unavailable: ${err.message}`;
  }
}
...
export default function GraphContactNexus() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const cs = normaliseContacts(ctData);
      setNodes(ns);
      setContacts(cs);
      setCrossed(crossRef(ns, cs));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:grcnt-toggle", onToggle);
    return () => window.removeEventListener("jarvis:grcnt-toggle", onToggle);
  }, [load]);
  ...
  const assess = useCallback(async () => {
    ...
    try {
      ...
      const unlinked   = total - associated;
      const coverage   = total > 0 ? Math.round((associated / total) * 100) : 0;
      const prompt = `Graph × Contact Nexus: ${coverage}% contact coverage (${associated}/${total} nodes linked to contacts, ${unlinked} unlinked). Assess the contact intelligence posture for influential graph entities in 2 sentences.`;
  ...
  const visible = crossed.filter((node) => {
    if (tab === "ASSOCIATED" && !node.associated) return false;
    if (tab === "UNLINKED"   &&  node.associated) return false;
    if (query) {
  ...
  const total      = crossed.length;
  const nAssoc     = crossed.filter((n) => n.associated).length;
  const nUnlinked  = total - nAssoc;
  const coverage   = total > 0 ? Math.round((nAssoc / total) * 100) : 0;
      ...
      {open && (
        <div style={panelStyle}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: "bold", letterSpacing: 1 }}>GRAPH × CONTACT NEXUS</span>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {[
              ...
              { label: "ASSOCIATED", value: nAssoc,          color: GRN },
              { label: "UNLINKED",   value: nUnlinked,       color: AMB },
              ...
              <div key={node.id}>
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${node.associated ? GRN + "44" : DIM + "22"}`,
                  }}
                >
                  <span style={{
                  ...
                  <span style={{ flex: 1, fontSize: 10, color: node.associated ? GRN : DIM, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {node.label}
                  </span>
                  ...
                  {node.associated ? (
                    <span style={{ fontSize: 8, color: GRN }}>⬡ {node.matches.length} ct</span>
                  ) : (
                    <span style={{ fontSize: 8, color: AMB }}>UNLINKED</span>
                  )}
                </div>

                {/* Expanded matched contacts */}
                {expanded === node.id && node.associated && (
                  <div style={{ marginLeft: 12, marginBottom: 6 }}>
                    ...
                    {node.matches.map((ct) => (
                      ...
                      >
                        <span style={{ color: GRN }}>{ct.name}</span>
                        {ct.role && (
                          <span style={{ color: CY, marginLeft: 4, fontSize: 8 }}>[{ct.role.slice(0, 20)}]</span>
                        )}
                        {ct.organisation && (
                          <span style={{ color: DIM, marginLeft: 4, fontSize: 8 }}>{ct.organisation.slice(0, 20)}</span>
                        )}
                        <span style={{ color: DIM, marginLeft: 6 }}>hits:{ct.hits}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {expanded === node.id && !node.associated && (
                  <div style={{ marginLeft: 12, marginBottom: 6, fontSize: 9, color: DIM }}>
                    No contacts correlate with this graph node.
