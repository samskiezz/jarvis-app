...
const API = '';
const OESC_RE = /\b(ops[._-]?spec|spec[._-]?ops|oesc|ops[._-]?events?[._-]?spec|documented[._-]?ops|ops[._-]?event[._-]?coverage|which[._-]?ops[._-]?events?[._-]?have[._-]?specs?|ops[._-]?spec[._-]?coverage|ops[._-]?spec[._-]?tracer)\b/i;

export function isOescQuery(t) {
  return OESC_RE.test(t || '');
}

export async function buildOescScript() {
  const [evR, spR] = await Promise.allSettled([
  ...
  const enriched = correlate(events, specs);
  const documented = enriched.filter(e => e._linked).length;
  const undocumented = enriched.length - documented;
  return (
    `OpsEvent × Spec Coverage: ${events.length} ops events, ${specs.length} specs indexed. ` +
    `${documented} events are backed by a spec; ${undocumented} have no spec coverage. ` +
    `Top undocumented: ${enriched.filter(e => !e._linked).slice(0, 4).map(e => e.event_type || e.title || e.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(event, spec) {
  const eToks = new Set([
  ...
  if (!eToks.size || !sToks.length) return 0;
  let hits = 0;
  for (const t of sToks) if (eToks.has(t)) hits++;
  return hits / Math.max(eToks.size, sToks.length);
}

function correlate(events, specs) {
  return events.map(event => {
    const scored = specs
      .map(spec => ({ spec, score: matchScore(event, spec) }))
      .filter(s => s.score > 0)
...
export default function OpsEventSpecCoverage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:oesc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:oesc-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${events.length} ops events and ${specs.length} specs. ${documented.length} events are backed by a spec; ${undocumented.length} have no spec coverage. Give a 2-sentence ops-spec coverage brief identifying the key governance gap and which undocumented event types most need spec backing.`,
        }),
      });
  ...
  const eventLabel = e => e.event_type || e.title || e.id || '?';

  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="OpsEvent × Spec Coverage Tracer (OESC)"
        style={{
          position: 'fixed', left: 743440, bottom: 8, zIndex: 257,
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {event.severity && chip(event.severity, event.severity === 'high' || event.severity === 'critical' ? RD : AM)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {event._matches.length > 0 ? (
                        <>
                          ...
                          {event._matches.map(({ spec, score }, j) => {
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No specs matched this event.</div>
                      )}
                    </div>
                  )}
                </div>
