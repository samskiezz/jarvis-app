/**
 * LiveIntelKnowledgeCoverage — F204
 *
 * Parallel-fetches /functions/getLiveIntel (quakes/crypto/FX) + /knowledge/
 * then keyword-correlates each live world event against KB articles to surface:
 *   DOCUMENTED   (≥1 KB match) — live event type has knowledge backing
 *   UNDOCUMENTED (0 matches)   — knowledge gap for the world event type
 *
 * Stat tiles: events / articles / documented / undocumented
 * Filter tabs: ALL / DOCUMENTED / UNDOCUMENTED
 * Text search.
 * Expand event → matched KB article cards with relevance bar.
 * ▶ ASSESS COVERAGE GAPS → /v1/jarvis/agent/chat 2-sentence brief + TTS.
 * 60 s auto-refresh.
 *
 * Intent: "liknow" / "live intel knowledge" / "world knowledge" /
 *         "world kb" / "event knowledge" / "undocumented events" /
 *         "live world knowledge" / "live knowledge gap"
 *   → jarvis:liknow-toggle + TTS brief via buildLiknowScript()
 *
 * Toggle: ◈ LIKNOW at left:36280, bottom:8, zIndex:104.
 * Mounted in App.jsx.
 */
...
const LIKNOW_RE =
  /\b(liknow|live.intel.knowledge|world.knowledge|world.kb|event.knowledge|undocumented.events|live.world.knowledge|live.knowledge.gap|intel.knowledge.gap|live.intel.kb)\b/i;

export function isLiknowQuery(t) { return LIKNOW_RE.test(t || ""); }

export async function buildLiknowScript() {
  ...
  const pairs    = correlate(events, articles);
  const documented   = pairs.filter((p) => p.matches.length >= 1).length;
  const undocumented = pairs.filter((p) => p.matches.length === 0).length;
  const topGaps = pairs
    ...
    .join("; ") || "none";
  return (
    `Assess JARVIS live intel knowledge coverage in 2 sentences. ` +
    `${events.length} live world events vs ${articles.length} KB articles: ` +
    `${documented} DOCUMENTED (KB backing exists), ` +
    `${undocumented} UNDOCUMENTED (no knowledge coverage). ` +
    `Top knowledge gaps: ${topGaps}.`
  );
}
...
function normaliseEvents(data) {
  ...
  quakes.forEach((q, i) => {
    events.push({
      ...
      keywords: `seismic earthquake ${q.place || ""} magnitude disaster geologic tremor tectonic geology fault`.toLowerCase(),
    });
  });
  ...
  coins.slice(0, 10).forEach((c, i) => {
    ...
    events.push({
      ...
      keywords: `crypto ${sym} ${sym.toLowerCase()} digital asset market finance blockchain currency defi token`.toLowerCase(),
    });
  });
  ...
  fx.slice(0, 8).forEach((f, i) => {
    ...
    events.push({
      ...
      keywords: `forex fx ${pair} ${pair.toLowerCase()} currency exchange rate finance market monetary`.toLowerCase(),
    });
  });
...
function normaliseArticles(raw) {
  return normaliseArray(raw, ["articles", "knowledge", "items", "data"]).map((a) => ({
    id:      a.id || a.article_id || String(Math.random()),
    title:   a.title || a.name || a.heading || "Untitled Article",
    summary: a.summary || a.abstract || a.content?.slice?.(0, 200) || "",
    tags:    [...(a.tags || []), ...(a.categories || [])].map(String),
...
function matchScore(event, article) {
  ...
  const artText = `${article.title} ${article.summary} ${article.tags.join(" ")} ${article.category}`.toLowerCase();
  const hits = evWords.filter((w) => artText.includes(w));
...
function correlate(events, articles) {
  return events.map((event) => {
    const scored = articles
      .map((a) => ({ a, score: matchScore(event, a) }))
...
export default function LiveIntelKnowledgeCoverage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPairs(correlate(events, articles));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:liknow-toggle", onToggle);
    return () => window.removeEventListener("jarvis:liknow-toggle", onToggle);
  }, []);
  ...
  const undocumented = pairs.filter((p) => p.matches.length === 0);

  const visible = pairs
    .filter((p) => {
      if (tab === "DOCUMENTED")   return p.matches.length >= 1;
      if (tab === "UNDOCUMENTED") return p.matches.length === 0;
      return true;
    })
    .filter((p) => {
  ...
  async function assess() {
    setAssessing(true);
    try {
      const script = await buildLiknowScript();
  ...
  if (!open) {
    return (
      <button
        ...
        title="Live Intel × Knowledge Coverage (LIKNOW)"
        style={{
          position: "fixed", left: BTN_LEFT, bottom: 8, zIndex: 104,
  ...
  const tabColor = (t) => {
    ...
    if (t === "UNDOCUMENTED") return AMBER;
    return CY;
  };

  return (
    <div style={{
      position: "fixed", left: BTN_LEFT - 200, bottom: 48, zIndex: 104,
      ...
      <div style={{ overflowY: "auto", flex: 1, padding: "0 12px 12px" }}>
        ...
        {visible.map((p) => {
          const status = p.matches.length >= 1 ? "DOCUMENTED" : "UNDOCUMENTED";
            ...
            >
              ...
              >
                ...
                <span style={{ color: DIM, fontSize: 8, flexShrink: 0 }}>
                  {p.event.type} · {p.matches.length} kb
                </span>
                ...
                <div style={{ paddingLeft: 16, paddingBottom: 4 }}>
                  ...
                  {p.matches.length === 0 ? (
                    <div style={{ color: AMBER, fontSize: 9 }}>
                      ◎ no KB article covers this live event — UNDOCUMENTED
                    </div>
                  ) : (
      ...
      <div style={{
        ...
        <span>LIKNOW · /functions/getLiveIntel × /knowledge/</span>
        <span
          onClick={load}
