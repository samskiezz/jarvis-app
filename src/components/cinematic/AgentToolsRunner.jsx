/**
 * F33 — Agent Tools Command Runner
...
const ATR_RE = /\b(agent tools|run tool|tool runner|tool palette|tools runner|jarvis tools|atr|show tools|list tools|open tools)\b/i;

export function isAgentToolsRunnerQuery(text) {
  return ATR_RE.test(text || "");
}

export async function buildAgentToolsRunnerScript() {
  const headers = {
    "Content-Type": "application/json",
    Authorization: `Bearer ${API_KEY}`,
  };
  try {
    const r = await fetch(`${apiBase()}/v1/jarvis/agent/tools`, { headers });
    if (!r.ok) return "Agent tools catalogue is standing by, sir.";
    const body = await r.json();
    const list = Array.isArray(body) ? body
      : Array.isArray(body.tools) ? body.tools
      : Array.isArray(body.data)  ? body.data
      : [];
    const n = list.length;
    if (!n) return "No tools are currently registered in my catalogue, sir.";
    const sample = list.slice(0, 3).map((t) => t.name || t.tool_name || "?").join(", ");
    return `Agent tools catalogue: ${n} tool${n !== 1 ? "s" : ""} available. Including: ${sample}${n > 3 ? " and more" : ""}. Use Control-Shift-T to open the runner, sir.`;
  } catch {
    return "Agent tools catalogue is standing by, sir.";
  }
}
...
export default function AgentToolsRunner() {
  ...
  // Voice / custom-event toggle (dispatched by JarvisBrain)
  useEffect(() => {
    const onToggle = () => setOpen((o) => !o);
    window.addEventListener("jarvis:atr-toggle", onToggle);
    return () => window.removeEventListener("jarvis:atr-toggle", onToggle);
  }, []);
