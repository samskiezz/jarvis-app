...
const API = '';
const TDCV_RE = /\b(task[._-]?decis|decis[._-]?task|tdcv|task[._-]?coverage[._-]?decis|tasks?[._-]?with[._-]?decis|uncovered[._-]?tasks?|task[._-]?decis[._-]?gap|tasks?[._-]?without[._-]?decis|task[._-]?decis[._-]?cov)\b/i;

export function isTdcvQuery(t) {
  return TDCV_RE.test(t || '');
}

export async function buildTdcvScript() {
  const [taskR, decR] = await Promise.allSettled([
  ...
  const decisions = normaliseDecisions(decR.status === 'fulfilled' ? decR.value : []);
  const enriched = correlate(tasks, decisions);
  const covered = enriched.filter(t => t._linked).length;
  const uncovered = enriched.length - covered;
  return (
    `Task × Decision Coverage: ${tasks.length} tasks, ${decisions.length} decisions indexed. ` +
    `${covered} tasks have decision backing; ${uncovered} have no governance coverage. ` +
    `Uncovered: ${enriched.filter(t => !t._linked).slice(0, 4).map(t => t.title || t.name || t.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(task, decision) {
  ...
  const decToks = [
    ...
    ...tokens(decision.context),
    ...tokens(decision.outcome),
  ].filter(Boolean);
  if (!taskToks.size || !decToks.length) return 0;
  let hits = 0;
  for (const t of decToks) if (taskToks.has(t)) hits++;
  return hits / Math.max(taskToks.size, decToks.length);
}

function correlate(tasks, decisions) {
  return tasks.map(task => {
    const scored = decisions
      .map(dec => ({ dec, score: matchScore(task, dec) }))
      .filter(s => s.score > 0)
...
const STATUS_COLOR = { final: GR, reviewed: CY, draft: AM };

export default function TaskDecisionCoverage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDecisions(normaliseDecisions(decR.status === 'fulfilled' ? decR.value : []));
    } catch { /* silently skip */ }
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:tdcv-toggle', onToggle);
    return () => window.removeEventListener('jarvis:tdcv-toggle', onToggle);
  }, []);
  ...
  const uncovered = enriched.filter(t => !t._linked);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${tasks.length} tasks and ${decisions.length} decisions. ${covered.length} tasks have decision backing; ${uncovered.length} have no governance coverage. Give a 2-sentence task-planning coverage brief highlighting the governance gap.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Task × Decision Coverage (TDCV)"
        style={{
          position: 'fixed', left: 671040, bottom: 8, zIndex: 241,
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              ...
              { label: 'DECISIONS', val: decisions.length, col: VI },
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const isCov = task._linked;
              const statusColor = isCov ? GR : AM;
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {chip(isCov ? 'COVERED' : 'UNCOVERED', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {task._matches.length > 0 ? (
                        <>
                          ...
                          {task._matches.map(({ dec, score }, j) => {
                            const decStatus = String(dec.status || '').toLowerCase();
                            const decColor = STATUS_COLOR[decStatus] || CY;
                            const quality = dec.quality_score != null ? Number(dec.quality_score) : null;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {dec.status && chip(dec.status, decColor)}
                                <span style={{ color: '#DCEBF5', fontSize: 10, flex: 1 }}>
                                  {dec.title || dec.id || '?'}
                                </span>
                                {quality != null && (
                                  <span style={{ color: VI, fontSize: 10 }}>q={quality.toFixed(2)}</span>
                                )}
                                {scorebar(score, GR)}
                              </div>
                            );
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No decisions matched this task — governance gap.</div>
                      )}
                    </div>
                  )}
                </div>
