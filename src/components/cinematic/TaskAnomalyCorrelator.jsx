...
const API = '';
const TACR_RE = /\b(task[._-]?anom|anom[._-]?task|tacr|exposed[._-]?tasks?|tasks?[._-]?with[._-]?anom|task[._-]?metric|metric[._-]?task|which[._-]?tasks?[._-]?have[._-]?anom|task[._-]?z[._-]?score|task[._-]?anomaly|anomaly[._-]?task[._-]?corr)\b/i;

export function isTaskAnomalyQuery(t) {
  return TACR_RE.test(t || '');
}

export async function buildTaskAnomalyScript() {
  const [taskR, anomR] = await Promise.allSettled([
  ...
  const enriched = correlate(tasks, anomalies);
  const exposed = enriched.filter(t => t._linked).length;
  const clear = enriched.length - exposed;
  return (
    `Task × Anomaly Correlation: ${tasks.length} tasks, ${anomalies.length} metric anomalies indexed. ` +
    `${exposed} tasks are exposed by active metric anomalies; ${clear} appear clear. ` +
...
function matchScore(task, anomaly) {
  ...
  if (!taskToks.size || !anomToks.length) return 0;
  let hits = 0;
  for (const t of anomToks) if (taskToks.has(t)) hits++;
  return hits / Math.max(taskToks.size, anomToks.length);
}

function correlate(tasks, anomalies) {
  return tasks.map(task => {
    const scored = anomalies
      .map(anom => ({ anom, score: matchScore(task, anom) }))
      .filter(s => s.score > 0)
...
export default function TaskAnomalyCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:tacr-toggle', onToggle);
    return () => window.removeEventListener('jarvis:tacr-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${tasks.length} tasks and ${anomalies.length} metric anomalies. ${exposed.length} tasks are exposed by active anomalies; ${clear.length} appear clear. Give a 2-sentence task anomaly brief with the key risk pattern.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Task × Anomaly Correlator (TACR)"
        style={{
          position: 'fixed', left: 593520, bottom: 8, zIndex: 223,
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {task._matches.length > 0 ? (
                        <>
                          ...
                          {task._matches.map(({ anom, score }, j) => {
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No anomalies matched this task.</div>
                      )}
                    </div>
                  )}
                </div>
