...
const API = '';
const INVDC_RE = /\b(investigation[._-]?decision|decision[._-]?investigation|invdc|case[._-]?decision|which[._-]?investigations[._-]?have[._-]?decisions|investigation[._-]?decision[._-]?coverage|investigation[._-]?governance|cases[._-]?without[._-]?decisions|decision[._-]?backed[._-]?investigation)\b/i;

export function isInvdcQuery(t) {
  return INVDC_RE.test(t || '');
}

export async function buildInvdcScript() {
  const [invR, decR] = await Promise.allSettled([
  ...
  const enriched = correlate(investigations, decisions);
  const covered = enriched.filter(i => i._linked).length;
  const uncovered = enriched.length - covered;
  return (
    `Investigation × Decision Coverage: ${investigations.length} cases, ${decisions.length} decisions indexed. ` +
    `${covered} investigations have decision governance; ${uncovered} remain ungoverned. ` +
    `Top ungoverned: ${enriched.filter(i => !i._linked).slice(0, 4).map(i => i.title || i.name || i.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(investigation, decision) {
  const iToks = new Set([
...
function correlate(investigations, decisions) {
  return investigations.map(inv => {
    const scored = decisions
      .map(dec => ({ dec, score: matchScore(inv, dec) }))
      .filter(s => s.score > 0)
...
const DEC_COLOR = { final: GR, approved: GR, reviewed: CY, draft: AM, deprecated: RD, published: GR };
...
export default function InvestigationDecisionCoverage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:invdc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:invdc-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${investigations.length} investigations and ${decisions.length} decisions. ${covered.length} investigations have decision governance; ${uncovered.length} are ungoverned. Give a 2-sentence investigation-decision coverage brief identifying the key governance gap pattern and which investigation types most urgently need decision backing.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Investigation × Decision Coverage (INVDC)"
        style={{
          position: 'fixed', left: 793600, bottom: 8, zIndex: 268,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: AM, fontSize: 11, letterSpacing: 2, fontWeight: 700, textShadow: `0 0 12px ${AM}` }}>
              ◈ INVESTIGATION × DECISION COVERAGE
            </span>
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              ...
              { label: 'DECISIONS', val: decisions.length, col: AM },
              { label: 'COVERED', val: covered.length, col: GR },
              { label: 'UNCOVERED', val: uncovered.length, col: RD },
            ].map(({ label: l, val, col }) => (
              <div key={l} style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = inv._linked ? GR : RD;
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {inv._matches.length > 0 ? (
                        <>
                          ...
                          {inv._matches.map(({ dec, score }, j) => {
                            const dCol = DEC_COLOR[String(dec.status || '').toLowerCase()] || AM;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {dec.status && chip(dec.status, dCol)}
                                <span style={{ color: '#DCEBF5', fontSize: 10, flex: 1 }}>
                                  {dec.title || dec.id || '?'}
                                </span>
                                {dec.quality_score != null && (
                                  <span style={{ color: '#6E8AA0', fontSize: 9 }}>
                                    q={Number(dec.quality_score).toFixed(2)}
                                  </span>
                                )}
                                {scorebar(score, GR)}
                              </div>
                        ...
                        <div style={{ color: RD, fontSize: 10 }}>No decisions matched this investigation.</div>
                      )}
                      {inv.description && (
