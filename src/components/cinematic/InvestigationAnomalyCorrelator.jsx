...
const API = '';
const IAVR_RE = /\b(investigation[._-]?anomaly|anomaly[._-]?investigation|iavr|case[._-]?anomaly|investigation[._-]?backed|unbacked[._-]?investigation|cases[._-]?with[._-]?anomalies|which[._-]?investigations[._-]?have[._-]?anomalies|case[._-]?anomaly[._-]?correlator)\b/i;

export function isIavrQuery(t) {
  return IAVR_RE.test(t || '');
}

export async function buildIavrScript() {
  const [invR, anR] = await Promise.allSettled([
  ...
  const anomalies = normaliseAnomalies(anR.status === 'fulfilled' ? anR.value : []);
  const enriched = correlate(investigations, anomalies);
  const backed = enriched.filter(i => i._linked).length;
  const unbacked = enriched.length - backed;
  return (
    `Investigation × Anomaly Correlator: ${investigations.length} cases, ${anomalies.length} anomalies indexed. ` +
    `${backed} investigations have metric-anomaly backing; ${unbacked} cases are flying blind with no anomaly data. ` +
    `Top unbacked: ${enriched.filter(i => !i._linked).slice(0, 4).map(i => i.title || i.name || i.id || '?').join(', ') || 'none'}.`
  );
}
...
function normaliseAnomalies(raw) {
  if (!raw) return [];
  if (Array.isArray(raw)) return raw;
  for (const k of ['anomalies', 'items', 'results', 'data', 'records']) {
    if (Array.isArray(raw[k])) return raw[k];
  }
  return [];
}
...
function matchScore(investigation, anomaly) {
  const iToks = new Set([
  ...
  if (!iToks.size || !aToks.length) return 0;
  let hits = 0;
  for (const t of aToks) if (iToks.has(t)) hits++;
  return hits / Math.max(iToks.size, aToks.length);
}

function correlate(investigations, anomalies) {
  return investigations.map(inv => {
    const scored = anomalies
      .map(anomaly => ({ anomaly, score: matchScore(inv, anomaly) }))
      .filter(s => s.score > 0)
...
const SEVERITY_COLOR = { critical: RD, high: RD, medium: AM, low: GR, info: CY };

export default function InvestigationAnomalyCorrelator() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setAnomalies(normaliseAnomalies(anR.status === 'fulfilled' ? anR.value : []));
    } catch { /* silently skip */ }
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:iavr-toggle', onToggle);
    return () => window.removeEventListener('jarvis:iavr-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(investigations, anomalies);
  const backed = enriched.filter(i => i._linked);
  const unbacked = enriched.filter(i => !i._linked);
  const badgeCount = unbacked.length;
  const badgeColor = badgeCount > 0 ? AM : GR;

  const filtered = enriched
    .filter(i => tab === 'ALL' || (tab === 'BACKED' ? i._linked : !i._linked))
    .filter(i => {
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${investigations.length} investigations and ${anomalies.length} anomalies. ${backed.length} cases are backed by metric anomaly data; ${unbacked.length} cases have no anomaly coverage. Give a 2-sentence case health brief identifying the most concerning unbacked investigation and what anomaly monitoring should be added.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Investigation × Anomaly Correlator (IAVR)"
        style={{
          position: 'fixed', left: 770800, bottom: 8, zIndex: 263,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2, fontWeight: 700, textShadow: `0 0 12px ${CY}` }}>
              ⊠ INVESTIGATION × ANOMALY CORRELATOR
            </span>
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              { label: 'CASES', val: investigations.length, col: CY },
              { label: 'ANOMALIES', val: anomalies.length, col: PR },
              { label: 'BACKED', val: backed.length, col: GR },
              { label: 'UNBACKED', val: unbacked.length, col: AM },
            ].map(({ label: l, val, col }) => (
              <div key={l} style={{
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {chip(inv._linked ? 'BACKED' : 'UNBACKED', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {inv.description && (
                        <div style={{ color: '#6E8AA0', fontSize: 10, marginBottom: 6 }}>
                          {String(inv.description).slice(0, 120)}{inv.description.length > 120 ? '…' : ''}
                        </div>
                      )}
                      {inv._matches.length > 0 ? (
                        <>
                          ...
                          {inv._matches.map(({ anomaly, score }, j) => {
                            const sev = String(anomaly.severity || anomaly.level || '').toLowerCase();
                            const sevColor = SEVERITY_COLOR[sev] || CY;
                            const metric = anomaly.metric || anomaly.metric_name || anomaly.name || anomaly.id || '?';
                            const zscore = anomaly.z_score != null ? anomaly.z_score : anomaly.zscore;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {sev && chip(sev, sevColor)}
                                {zscore != null && (
                                  <span style={{ color: '#6E8AA0', fontSize: 9 }}>
                                    z={Number(zscore).toFixed(2)}
                                  </span>
                                )}
                                <span style={{ color: '#DCEBF5', fontSize: 10, flex: 1 }}>{metric}</span>
                                {scorebar(score, GR)}
                              </div>
                            );
                          })}
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No anomalies matched this investigation.</div>
                      )}
                    </div>
                  )}
                </div>
