/**
 * RemindersDatasetNexus — F604
 * "JARVIS, reminders dataset / dataset reminders / remdset / data-linked reminders / floating data notes"
 * Cross-references /reminders/list against /v1/datasets.
 * DATA-LINKED reminders (≥1 dataset keyword-matches) vs FLOATING (no dataset backing).
 * Coverage % tile; ALL/DATA-LINKED/FLOATING filter tabs + search; click-to-expand matched datasets.
...
const REMDSET_RE =
  /\bremdset\b|\breminders?.dataset\b|\bdataset.?reminders?\b|\bdata.?linked.?reminders?\b|\bfloating.?data.?notes?\b|\breminder.?dataset.?coverage\b|\breminders?.with.?data(set)?\b/i;

export function isRemdsetQuery(text) {
  return REMDSET_RE.test(text || "");
}
...
function normaliseDatasets(data) {
  ...
  return arr.map((d, i) => ({
    ...
    rows:  typeof d.row_count === "number" ? d.row_count
         : typeof d.rows     === "number" ? d.rows
         : null,
    desc:  (d.description || d.summary || "").toString().slice(0, 150),
    tags:  Array.isArray(d.tags) ? d.tags.join(" ") : (d.tags || ""),
  }));
}

function crossRef(reminders, datasets) {
  return reminders.map((rem) => {
    const haystack = `${rem.content} ${(rem.tags || []).join(" ")}`;
    const matches = datasets
      .map((ds) => {
        const hits = overlap(haystack, `${ds.name} ${ds.desc} ${ds.tags}`);
        return hits > 0 ? { ...ds, hits } : null;
      })
      .filter(Boolean)
      .sort((a, b) => b.hits - a.hits)
      .slice(0, 5);
    return { ...rem, datasets: matches, linked: matches.length > 0 };
  });
}

export async function buildRemdsetScript() {
  try {
    const base = apiBase();
    const hdr = { Authorization: `Bearer ${API_KEY}` };
    const [remRes, dsRes] = await Promise.all([
      fetch(`${base}/reminders/list`, { headers: hdr }),
      fetch(`${base}/v1/datasets`,    { headers: hdr }),
    ]);
    const [remData, dsData] = await Promise.all([remRes.json(), dsRes.json()]);
    const reminders = normaliseReminders(remData);
    const datasets  = normaliseDatasets(dsData);
    const rows      = crossRef(reminders, datasets);
    ...
    const topFloating = rows
      ...
      .join("; ");
    return (
      `${linked} of ${rows.length} reminders are data-linked to known datasets (${pct}% coverage). ` +
      (floating > 0
        ? `${floating} floating reminder${floating !== 1 ? "s" : ""} have no matching dataset — ungrounded notes: ${topFloating || "unknown"}.`
        : "All reminders are backed by at least one dataset.")
    );
  } catch {
    return "Unable to reach reminders or datasets endpoints, sir.";
  }
}

export default function RemindersDatasetNexus() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [remData, dsData] = await Promise.all([remRes.json(), dsRes.json()]);
      setRows(crossRef(normaliseReminders(remData), normaliseDatasets(dsData)));
    } catch {
      /* non-fatal */
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:remdset-toggle", toggle);
    return () => window.removeEventListener("jarvis:remdset-toggle", toggle);
  }, []);
  ...
  const assess = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message:
            `Assess reminder-to-dataset linkage: ${rows.length} reminders total, ` +
            `${linked.length} are data-linked to known datasets, ` +
            `${floating.length} are floating with no associated dataset. ` +
            `Top floating: ${floating.slice(0, 3).map((r) => r.content.slice(0, 40)).join("; ") || "none"}. ` +
            "Give a 2-sentence operational-memory and data-coverage assessment with recommended action.",
        }),
      });
  ...
  const visible = rows
    .filter((r) => {
      if (tab === "DATA-LINKED") return r.linked;
  ...
  const DS_KIND_COLOR = {
    DATASET: CY, TABLE: GRN, VIEW: AMB, STREAM: RED, FILE: DIM,
  };

  const BTN_LEFT = 84_740;
  const BTN_STYLE = {
    position: "fixed",
    left: BTN_LEFT,
    bottom: 8,
  ...
  return (
    <>
      <button
        style={BTN_STYLE}
        onClick={() => setOpen((o) => !o)}
        title="Reminders × Dataset Nexus (REMDSET)"
      >
        ◈ REMDSET
        {floating > 0 && (
      ...
      {open && (
        <div style={PANEL}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2 }}>REMINDERS × DATASET NEXUS</span>
            <button
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, marginBottom: 10 }}>
            {[
              ...
              { label: "DATA-LINKED", value: linked,      color: linked > 0 ? GRN : DIM },
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
              <span style={{ color: DIM, fontSize: 9, letterSpacing: 1 }}>DATASET COVERAGE</span>
          ...
          <div style={{ display: "flex", gap: 5, marginBottom: 8 }}>
            {["ALL", "DATA-LINKED", "FLOATING"].map((t) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  ...
                  <span style={{ color: rem.linked ? GRN : DIM, fontSize: 9 }}>
                    {rem.linked ? `${rem.datasets.length} dataset${rem.datasets.length !== 1 ? "s" : ""}` : "FLOATING"}
                  </span>
                </div>
                  ...
                  <div style={{ marginTop: 6, borderTop: `1px solid ${CY}22`, paddingTop: 6 }}>
                    {rem.linked ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {rem.datasets.map((ds) => (
                          <div key={ds.id} style={{ background: "rgba(0,229,160,0.04)", border: `1px solid ${GRN}33`, borderRadius: 4, padding: "5px 7px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                              <span style={{ color: DS_KIND_COLOR[ds.kind] || CY, fontSize: 9, border: `1px solid ${(DS_KIND_COLOR[ds.kind] || CY)}44`, borderRadius: 3, padding: "1px 4px" }}>{ds.kind}</span>
                              <span style={{ color: "#DCEBF5", fontSize: 10, flex: 1 }}>{ds.name}</span>
                              {ds.rows !== null && (
                                <span style={{ color: DIM, fontSize: 9 }}>{ds.rows.toLocaleString()} rows</span>
                              )}
                              <span style={{ color: DIM, fontSize: 9, marginLeft: 4 }}>hits: {ds.hits}</span>
                            </div>
                            {ds.desc && (
                              <div style={{ color: DIM, fontSize: 9, marginTop: 3, lineHeight: 1.4 }}>{ds.desc.slice(0, 100)}{ds.desc.length > 100 ? "…" : ""}</div>
                            )}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{ color: DIM, fontSize: 10 }}>No datasets matched this reminder — note has no data grounding.</div>
                    )}
                  </div>
                )}
              </div>
