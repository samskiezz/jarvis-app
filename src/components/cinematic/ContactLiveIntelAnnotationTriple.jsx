...
const CLGANN_RE = /\b(clgann|contact\s+live\s+annotation|contact\s+world\s+annotation|live\s+annotation\s+contact|world\s+contact\s+annotation|contact\s+live\s+intel\s+annotation|live\s+intel\s+contact\s+annotation|dark\s+contact\s+live|contact\s+live\s+world|contact\s+annotation\s+live)\b/i;

export function isClgannQuery(t) {
  return CLGANN_RE.test(t || '');
}

export async function buildClgannScript() {
  try {
    ...
    const fullyAlarmed = classified.filter(c => c.state === 'FULLY_ALARMED').length;
    const worldTriggered = classified.filter(c => c.state === 'WORLD_TRIGGERED').length;
    const graphAnnotated = classified.filter(c => c.state === 'GRAPH_ANNOTATED').length;
    const dark = classified.filter(c => c.state === 'DARK').length;
    return `CLGANN contact-live-annotation coverage: ${contacts.length} contacts — ${fullyAlarmed} fully alarmed (live intel+annotation), ${worldTriggered} world-triggered only, ${graphAnnotated} graph-annotated only, ${dark} dark (neither). ${dark > 0 ? `${dark} contacts have no live world event or graph annotation linkage.` : 'All contacts have at least one active linkage.'}`;
  } catch {
    return 'CLGANN data unavailable — check /entities/Contact, /functions/getLiveIntel, and /v1/graph/annotations endpoints.';
  }
}
...
function normaliseContacts(raw) {
  ...
  return arr.map((c, i) => ({
    ...
    title: c.title || c.job_title || c.role || c.position || '',
...
function classifyContact(contact, liveEvents, annotations) {
  ...
  let state;
  if (matchedLive.length > 0 && matchedAnns.length > 0) state = 'FULLY_ALARMED';
...
const STATE_META = {
  FULLY_ALARMED:   { label: 'FULLY ALARMED',   color: '#f59e0b', bg: 'rgba(245,158,11,0.13)' },
  WORLD_TRIGGERED: { label: 'WORLD TRIGGERED',  color: '#fb923c', bg: 'rgba(251,146,60,0.13)'  },
  GRAPH_ANNOTATED: { label: 'GRAPH ANNOTATED',  color: '#22d3ee', bg: 'rgba(34,211,238,0.13)'  },
  DARK:            { label: 'DARK',             color: '#6b7280', bg: 'rgba(107,114,128,0.13)' },
};

const FILTERS = ['ALL', 'FULLY_ALARMED', 'WORLD_TRIGGERED', 'GRAPH_ANNOTATED', 'DARK'];

export default function ContactLiveIntelAnnotationTriple() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:clgann-toggle', onToggle);
    return () => window.removeEventListener('jarvis:clgann-toggle', onToggle);
  }, []);
  ...
  useEffect(() => {
    const onVoice = e => {
      if (CLGANN_RE.test(e.detail?.query || '')) setOpen(true);
    };
  ...
  const assess = useCallback(async () => {
    ...
    setAssessment('');
    try {
      const dark = classified.filter(c => c.state === 'DARK').map(c => c.name).slice(0, 5).join(', ');
      const alarmed = classified.filter(c => c.state === 'FULLY_ALARMED').map(c => c.name).slice(0, 3).join(', ');
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: `Assess CLGANN contact live-world-annotation coverage. ${contacts.length} contacts. Fully alarmed (live intel+annotation): ${alarmed || 'none'}. Dark (no live or annotation linkage): ${dark || 'none'}. In 2 sentences, identify the largest coverage gap and recommend the highest-priority intelligence action.`,
        }),
      });
  ...
  const counts = {
    FULLY_ALARMED:   classified.filter(c => c.state === 'FULLY_ALARMED').length,
    WORLD_TRIGGERED: classified.filter(c => c.state === 'WORLD_TRIGGERED').length,
    GRAPH_ANNOTATED: classified.filter(c => c.state === 'GRAPH_ANNOTATED').length,
  ...
  return (
    <div style={{
      position: 'fixed', left: 832480, bottom: 8, zIndex: 515,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ color: '#f59e0b', fontWeight: 700, letterSpacing: 1 }}>◈ CLGANN</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {counts.FULLY_ALARMED > 0 && (
            <span style={{
              background: 'rgba(245,158,11,0.25)', color: '#f59e0b',
              borderRadius: 4, padding: '1px 6px', fontSize: 10, fontWeight: 700,
            }}>{counts.FULLY_ALARMED} ALARMED</span>
          )}
          {counts.DARK > 0 && (
            <span style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 4, padding: '8px 12px', flexShrink: 0 }}>
        {[
          ...
          { label: 'ANNOTATIONS',   value: annotations.length,   color: '#22d3ee' },
          { label: 'FULLY ALARMED', value: counts.FULLY_ALARMED, color: '#f59e0b' },
          { label: 'WORLD TRIG',    value: counts.WORLD_TRIGGERED, color: '#fb923c' },
          { label: 'GRAPH ANNOT',   value: counts.GRAPH_ANNOTATED, color: '#22d3ee' },
          { label: 'DARK',          value: counts.DARK,           color: '#6b7280' },
          { label: 'COVERAGE',      value: `${Math.round(((total - counts.DARK) / total) * 100)}%`, color: '#f59e0b' },
        ].map(s => (
      ...
      <div style={{ padding: '0 12px 8px', flexShrink: 0 }}>
        <div style={{ height: 6, borderRadius: 3, background: '#1e293b', overflow: 'hidden', display: 'flex' }}>
          {(['FULLY_ALARMED','WORLD_TRIGGERED','GRAPH_ANNOTATED','DARK']).map(state => (
            <div key={state} style={{
            ...
            <div key={item.id} style={{
              ...
              {isExp && (
                <div style={{ padding: '0 8px 8px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                      ...
                      : item.matchedLive.slice(0, 5).map((e, idx) => (
                        <div key={e.id || idx} style={{ marginBottom: 3 }}>
                          ...
                          <div style={{ height: 3, borderRadius: 2, marginTop: 2, background: '#1e293b', overflow: 'hidden' }}>
                            <div style={{
                              width: `${Math.min(100, matchScore(tok([item.name,item.company,item.title,item.description,item.tags].join(' ')), [e.title,e.type,e.detail].join(' ')) * 100 / THRESHOLD)}%`,
                  ...
                  {/* Annotations */}
                  <div>
                    <div style={{ color: '#22d3ee', fontSize: 9, fontWeight: 700, marginBottom: 3 }}>
                    ...
                    {item.matchedAnns.length === 0
                      ...
                      : item.matchedAnns.slice(0, 5).map(a => (
                        <div key={a.id} style={{ marginBottom: 3 }}>
                          <div style={{ color: '#cbd5e1', fontSize: 10 }}>
                            {a.label || a.entity}
                            {a.targetType && <span style={{ color: '#22d3ee', fontSize: 9, marginLeft: 4 }}>[{a.targetType}]</span>}
                          </div>
                          <div style={{ height: 3, borderRadius: 2, marginTop: 2, background: '#1e293b', overflow: 'hidden' }}>
                            <div style={{
                              width: `${Math.min(100, matchScore(tok([item.name,item.company,item.title,item.description,item.tags].join(' ')), [a.label,a.entity,a.notes,a.tags].join(' ')) * 100 / THRESHOLD)}%`,
                              background: '#22d3ee', height: '100%',
                            }} />
                          </div>
                        </div>
                      ))
      ...
      <div style={{
        ...
        <button onClick={assess} disabled={assessing} style={{
          background: assessing ? 'rgba(245,158,11,0.06)' : 'rgba(245,158,11,0.14)',
