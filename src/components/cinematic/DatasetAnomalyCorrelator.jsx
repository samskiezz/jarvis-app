...
const API = '';
const DSAN_RE = /\b(dataset[._-]?anomal|anomal[._-]?dataset|dsan|datasets?[._-]?with[._-]?anomal|exposed[._-]?datasets?[._-]?anomal|which[._-]?datasets?[._-]?have[._-]?anomal|dataset[._-]?metric[._-]?anomal|dataset[._-]?anomal[._-]?correlat)\b/i;

export function isDsanQuery(t) {
  return DSAN_RE.test(t || '');
}

export async function buildDsanScript() {
  const [dsR, anR] = await Promise.allSettled([
  ...
  const enriched = correlate(datasets, anomalies);
  const exposed = enriched.filter(d => d._linked).length;
  const clear = enriched.length - exposed;
  return (
    `Dataset × Anomaly Correlation: ${datasets.length} datasets, ${anomalies.length} anomalies indexed. ` +
    `${exposed} datasets are matched to active anomalies; ${clear} appear clear. ` +
    `Top exposed: ${enriched.filter(d => d._linked).slice(0, 4).map(d => d.name || d.key || d.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(dataset, anomaly) {
  const dToks = new Set([
    ...tokens(dataset.name),
    ...tokens(dataset.key),
  ...
  if (!dToks.size || !aToks.length) return 0;
  let hits = 0;
  for (const t of aToks) if (dToks.has(t)) hits++;
  return hits / Math.max(dToks.size, aToks.length);
}

function correlate(datasets, anomalies) {
  return datasets.map(dataset => {
    const scored = anomalies
      .map(anomaly => ({ anomaly, score: matchScore(dataset, anomaly) }))
      .filter(s => s.score > 0)
...
export default function DatasetAnomalyCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:dsan-toggle', onToggle);
    return () => window.removeEventListener('jarvis:dsan-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(datasets, anomalies);
  const exposed = enriched.filter(d => d._linked);
  const clear = enriched.filter(d => !d._linked);
  const badgeCount = exposed.length;
  const badgeColor = badgeCount > 0 ? RD : GR;

  const filtered = enriched
    .filter(d => tab === 'ALL' || (tab === 'EXPOSED' ? d._linked : !d._linked))
    .filter(d => {
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${datasets.length} datasets and ${anomalies.length} active anomalies. ${exposed.length} datasets are matched to active anomalies; ${clear.length} appear clear. Give a 2-sentence dataset anomaly correlation brief identifying the key data quality risk and which datasets or metrics are most anomaly-correlated.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Dataset × Anomaly Correlator (DSAN)"
        style={{
          position: 'fixed', left: 738880, bottom: 8, zIndex: 256,
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = dataset._linked ? RD : GR;
              return (
                <div
                ...
                >
                  ...
                  >
                    ...
                    {chip(dataset._linked ? 'EXPOSED' : 'CLEAR', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {dataset._matches.length > 0 ? (
                        <>
                          ...
                          {dataset._matches.map(({ anomaly, score }, j) => {
                            const sev = String(anomaly.severity || anomaly.level || '').toLowerCase();
                            const sevColor = SEVERITY_COLOR[sev] || AM;
                            ...
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                ...
                                <span style={{ color: '#DCEBF5', fontSize: 10, flex: 1 }}>
                                  {anomaly.metric || anomaly.metric_name || anomaly.name || anomaly.id || '?'}
                                </span>
                                {scorebar(score, RD)}
                              </div>
                            );
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No anomalies matched this dataset.</div>
                      )}
                    </div>
                  )}
                </div>
