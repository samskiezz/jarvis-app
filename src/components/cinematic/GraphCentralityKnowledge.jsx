/**
 * GraphCentralityKnowledge — F96
 *
 * Parallel-fetches /v1/graph/centrality + /knowledge/ then keyword-
 * correlates high-centrality graph nodes against KB articles to surface
 * GROUNDED (≥2 article matches) / PARTIAL (1) / BARE (0 — knowledge gap
 * for the most influential nodes in the graph).
 *
 * Stat tiles: nodes / articles / grounded / partial / bare
 * Filter tabs: ALL / GROUNDED / PARTIAL / BARE
 * Expand node → matched KB article cards with relevance score bar.
 * Click ▶ ASSESS KNOWLEDGE GAPS → /v1/jarvis/agent/chat 2-sentence brief
 *   + jarvis:speak-dossier TTS.
 * 90 s auto-refresh.
 *
 * Intent: "graph knowledge" / "gcnk" / "node knowledge" /
 *         "centrality knowledge" / "bare nodes" / "graph kb" /
 *         "influential nodes knowledge" / "knowledge graph coverage"
 *   → jarvis:gcnk-toggle + TTS brief via buildGcknScript()
 *
 * Toggle: ◈ GCNK at left:31240, bottom:8, zIndex 96.
 * Mounted in App.jsx.
 */
...
const GCNK_RE =
  /\b(g[ck]n[k]?|graph.knowl|node.knowl|central.*knowl|knowl.*central|bare.node|graph.kb|influential.*knowl|knowl.*graph|gcnk)\b/i;

export function isGcknQuery(t) { return GCNK_RE.test(t || ""); }

export async function buildGcknScript() {
  const [cRaw, kRaw] = await Promise.allSettled([
  ...
  const pairs    = correlate(nodes, articles);
  ...
  const topBare  = pairs
    ...
    .join(", ") || "none";
  return (
    `Assess JARVIS graph-centrality knowledge coverage in 2 sentences. ` +
    `${nodes.length} high-centrality nodes vs ${articles.length} KB articles: ` +
    `${grounded} GROUNDED (≥2 articles), ${partial} PARTIAL (1 article), ` +
    `${bare} BARE (no KB backing — influential nodes with no knowledge). ` +
    `Top bare nodes: ${topBare}.`
  );
}
...
function normaliseCentrality(raw) {
  return normaliseArray(raw, ["nodes", "centrality", "entities"]).map((n) => ({
    id:    n.id || n.node_id || n.entity_id || String(Math.random()),
    label: n.label || n.name || n.entity || n.id || "Unknown",
    score: parseFloat(n.centrality ?? n.score ?? n.degree ?? 0),
    type:  n.type || n.entity_type || "",
    tags:  [...(n.tags || []), ...(n.labels || [])].map(String),
  }));
}
...
function matchScore(node, article) {
  const nodeWords = tokens(`${node.label} ${node.type} ${node.tags.join(" ")}`);
  const artText   = `${article.title} ${article.summary} ${article.tags.join(" ")}`.toLowerCase();
  const hits = nodeWords.filter((w) => artText.includes(w));
  return hits.length / Math.max(nodeWords.length, 1);
}

function correlate(nodes, articles) {
  return nodes.map((node) => {
    const scored = articles
      .map((a) => ({ a, score: matchScore(node, a) }))
...
export default function GraphCentralityKnowledge() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cRes, kRes] = await Promise.allSettled([
        fetch(`${apiBase()}/v1/graph/centrality`, {
          headers: { Authorization: `Bearer ${API_KEY}` },
      ...
      setPairs(correlate(nodes, articles));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:gcnk-toggle", onToggle);
    return () => window.removeEventListener("jarvis:gcnk-toggle", onToggle);
  }, []);
  ...
  async function assess() {
    setAssessing(true);
    try {
      const script = await buildGcknScript();
  ...
  if (!open) {
    return (
      <button
        ...
        title="Graph Centrality × Knowledge Coverage (GCNK)"
        style={{
          position: "fixed", left: BTN_LEFT, bottom: 8, zIndex: 96,
  ...
  return (
    <div style={{
      position: "fixed", left: BTN_LEFT - 200, bottom: 48, zIndex: 96,
      ...
      <div style={{ overflowY: "auto", flex: 1, padding: "0 12px 12px" }}>
        ...
        {visible.map((p) => {
          ...
          const isExp = expanded[p.node.id];
          return (
            <div
              key={p.node.id}
              style={{
      ...
      <div style={{
        ...
        <span>GCNK · /v1/graph/centrality × /knowledge/</span>
        <span
          onClick={load}
