...
const API = '';
const CTBD_RE = /\b(contact[._-]?task|task[._-]?assignment|team[._-]?workload|ctbd|contact[._-]?workload|who[._-]?is[._-]?working[._-]?on|assigned[._-]?contacts?|task[._-]?load|contact[._-]?tasks?|team[._-]?task[._-]?board|task[._-]?owners?)\b/i;

export function isCtbdQuery(t) {
  return CTBD_RE.test(t || '');
}

export async function buildCtbdScript() {
  const [conR, taskR] = await Promise.allSettled([
    fetch(`${API}/entities/Contact`).then(r => r.json()),
    fetch(`${API}/entities/Task`).then(r => r.json()),
  ]);
  const contacts = normaliseArray(conR.status === 'fulfilled' ? conR.value : []);
  const tasks = normaliseArray(taskR.status === 'fulfilled' ? taskR.value : []);
  const enriched = correlate(contacts, tasks);
  const assigned = enriched.filter(c => c._linked).length;
  const unassigned = enriched.filter(c => !c._linked).length;
  const topAssigned = enriched
    .filter(c => c._linked)
    .slice(0, 4)
    .map(c => c.name || c.title || c.email || '?')
    .join(', ');
  return (
    `Contact × Task Board: ${contacts.length} contacts, ${tasks.length} tasks indexed. ` +
    `${assigned} contacts have active task assignments; ${unassigned} have no matched tasks. ` +
    `Most active contacts: ${topAssigned || 'none found'}.`
  );
}
...
function normaliseArray(raw) {
  ...
  for (const k of ['items', 'results', 'data', 'contacts', 'tasks', 'entities', 'records']) {
...
function matchScore(contact, task) {
  ...
  const taskToks = [
    ...
    ...tokens(task.owner),
    ...tokens(task.assigned_to),
  ].filter(Boolean);
  if (!conToks.size || !taskToks.length) return 0;
  let hits = 0;
  for (const t of taskToks) if (conToks.has(t)) hits++;
  return hits / Math.max(conToks.size, taskToks.length);
}

function correlate(contacts, tasks) {
  return contacts.map(contact => {
    const scored = tasks
      .map(task => ({ task, score: matchScore(contact, task) }))
      .filter(s => s.score > 0)
...
const STATUS_COLOR = {
  open: CY, in_progress: AM, done: GR, closed: GR,
  blocked: '#EF4444', pending: '#6E8AA0',
};
...
export default function ContactTaskBoard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setContacts(normaliseArray(conR.status === 'fulfilled' ? conR.value : []));
      setTasks(normaliseArray(taskR.status === 'fulfilled' ? taskR.value : []));
    } catch { /* silently skip */ }
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:ctbd-toggle', onToggle);
    return () => window.removeEventListener('jarvis:ctbd-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(contacts, tasks);
  const assigned = enriched.filter(c => c._linked);
  const unassigned = enriched.filter(c => !c._linked);
  const badgeCount = assigned.length;
  const badgeColor = badgeCount > 0 ? GR : '#6E8AA0';

  const filtered = enriched
    .filter(c => tab === 'ALL' || (tab === 'ASSIGNED' ? c._linked : !c._linked))
    .filter(c => {
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${contacts.length} contacts and ${tasks.length} tasks. ${assigned.length} contacts have matched task assignments; ${unassigned.length} have no matched tasks. Give a 2-sentence team workload brief highlighting who is most loaded and any contacts with no active tasks.`,
        }),
      });
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        title="Contact × Task Assignment Board (CTBD)"
        style={{
          position: 'fixed', left: 534240, bottom: 8, zIndex: 210,
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              ...
              { label: 'ASSIGNED', val: assigned.length, col: GR },
              { label: 'UNASSIGNED', val: unassigned.length, col: AM },
            ].map(({ label: l, val, col }) => (
              <div key={l} style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = contact._linked ? GR : AM;
              return (
                <div
                  key={contact.id || i}
                ...
                >
                  ...
                  >
                    ...
                    {contact.title && chip(contact.title, PU)}
                    {chip(contact._linked ? 'ASSIGNED' : 'UNASSIGNED', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {contact._matches.length > 0 ? (
                        <>
                          ...
                          {contact._matches.map(({ task, score }, j) => {
                            const sc = STATUS_COLOR[String(task.status || '').toLowerCase()] || CY;
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                {task.status && chip(task.status, sc)}
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No task assignments matched this contact.</div>
                      )}
                      {contact.email && (
                        <div style={{ color: '#6E8AA0', fontSize: 9, marginTop: 4 }}>
                          ✉ {contact.email}
                        </div>
                      )}
                    </div>
