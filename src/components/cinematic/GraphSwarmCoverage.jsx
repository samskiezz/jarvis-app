/**
 * GraphSwarmCoverage — F584
 * "JARVIS, graph swarm / swarm graph / grswrm / automated nodes / unautomated nodes / node swarm coverage"
 * Cross-references /v1/graph/centrality (top influential nodes) + /entities/SwarmJob.
 * Finds AUTOMATED nodes (≥1 swarm job keyword-matches) vs UNAUTOMATED (no swarm backing).
 * Coverage % tile; ALL/AUTOMATED/UNAUTOMATED filter tabs + search; click-to-expand matched jobs.
 * ▶ ASSESS → /v1/jarvis/agent/chat 2-sentence graph-swarm intelligence brief + TTS.
...
const BTN_LEFT = 71_980;
const Z_INDEX  = 142;

const GRSWRM_RE =
  /\bgrswrm\b|\bgraph.?swarm\b|\bswarm.?graph\b|\bautomated.?nodes?\b|\bunautomated.?nodes?\b|\bnode.?swarm.?coverage\b|\bgraph.?automation\b|\bwhich.?nodes?.?have.?swarm\b|\bnode.?swarm\b|\bgraph.?swarm.?coverage\b/i;

export function isGrswrmQuery(text) {
  return GRSWRM_RE.test(text || "");
}
...
function normaliseJobs(raw) {
  ...
  return arr.map((j, i) => ({
    ...
    kind:    (j.kind   || j.type      || j.job_type || "SWARM").toString().toUpperCase(),
    target:  j.target  || j.target_entity || j.entity || "",
    progress: typeof j.progress === "number" ? j.progress : null,
    tags:    Array.isArray(j.tags) ? j.tags.join(" ") : (j.tags || ""),
  }));
}

function crossRef(nodes, jobs) {
  return nodes.map((node) => {
    const haystack = `${node.label} ${node.description} ${node.tags} ${node.kind}`;
    const matches = jobs
      .map((job) => ({
        job,
        hits: overlap(haystack, `${job.name} ${job.target} ${job.kind} ${job.tags}`),
      }))
      .filter(({ hits }) => hits > 0)
      .sort((a, b) => b.hits - a.hits);
    return {
      ...node,
      automated: matches.length > 0,
      matches: matches.map(({ job, hits }) => ({ ...job, hits })),
    };
  });
}

// ─── buildGrswrmScript (for JarvisBrain) ─────────────────────────────────────

export async function buildGrswrmScript() {
  try {
    const base = apiBase();
    ...
    const crossed = crossRef(nodes, jobs);

    const total        = crossed.length;
    const automated    = crossed.filter((n) => n.automated).length;
    const unautomated  = total - automated;
    const coverage     = total > 0 ? Math.round((automated / total) * 100) : 0;
    const topDark      = crossed
      .filter((n) => !n.automated)
      .slice(0, 2)
      .map((n) => n.label)
      .join(", ");

    const brief =
      `${coverage}% of ${total} top graph nodes have swarm job coverage. ` +
      `${automated} AUTOMATED, ${unautomated} UNAUTOMATED.` +
      (topDark ? ` Top unautomated nodes: ${topDark}.` : "");

    const agentRes = await fetch(`${base}/v1/jarvis/agent/chat`, {
      method: "POST",
      headers: { ...hdr, "Content-Type": "application/json" },
      body: JSON.stringify({
        message: `Graph × Swarm Coverage: ${brief} Provide a 2-sentence automation readiness assessment for high-influence graph entities.`,
      }),
    });
    ...
    return `Graph × Swarm Coverage unavailable: ${err.message}`;
  }
}
...
export default function GraphSwarmCoverage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const js = normaliseJobs(jobData);
      setNodes(ns);
      setJobs(js);
      setCrossed(crossRef(ns, js));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:grswrm-toggle", onToggle);
    return () => window.removeEventListener("jarvis:grswrm-toggle", onToggle);
  }, [load]);
  ...
  const assess = useCallback(async () => {
    ...
    try {
      ...
      const unautomated = total - automated;
      const coverage    = total > 0 ? Math.round((automated / total) * 100) : 0;
      const prompt = `Graph × Swarm Coverage: ${coverage}% automation coverage (${automated}/${total} nodes have swarm jobs, ${unautomated} unautomated). Assess automation posture for high-influence graph entities in 2 sentences.`;
  ...
  const visible = crossed.filter((node) => {
    if (tab === "AUTOMATED"   && !node.automated) return false;
    if (tab === "UNAUTOMATED" &&  node.automated) return false;
    if (query) {
  ...
  const total       = crossed.length;
  const nAuto       = crossed.filter((n) => n.automated).length;
  const nUnauto     = total - nAuto;
  const coverage    = total > 0 ? Math.round((nAuto / total) * 100) : 0;
      ...
      {open && (
        <div style={panelStyle}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: "bold", letterSpacing: 1 }}>GRAPH × SWARM COVERAGE</span>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {[
              ...
              { label: "AUTOMATED",    value: nAuto,          color: GRN },
              { label: "UNAUTOMATED",  value: nUnauto,        color: AMB },
              ...
              <div key={node.id}>
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${node.automated ? GRN + "44" : DIM + "22"}`,
                  }}
                >
                  <span style={{
                  ...
                  <span style={{ flex: 1, fontSize: 10, color: node.automated ? GRN : DIM, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {node.label}
                  </span>
                  ...
                  {node.automated ? (
                    <span style={{ fontSize: 8, color: GRN }}>⬡ {node.matches.length} job{node.matches.length !== 1 ? "s" : ""}</span>
                  ) : (
                    <span style={{ fontSize: 8, color: AMB }}>UNAUTO</span>
                  )}
                </div>

                {/* Expanded matched swarm jobs */}
                {expanded === node.id && node.automated && (
                  <div style={{ marginLeft: 12, marginBottom: 6 }}>
                    {node.description && (
                      <div style={{ fontSize: 9, color: DIM, marginBottom: 4 }}>{node.description.slice(0, 120)}</div>
                    )}
                    {node.matches.map((job) => (
                      <div
                        key={job.id}
                        style={{
                      ...
                      >
                        <span style={{ color: GRN }}>{job.name}</span>
                        <span style={{ color: STATUS_COLOR[job.status] || AMB, marginLeft: 4, fontSize: 8 }}>[{job.status}]</span>
                        {job.target && <span style={{ color: DIM, marginLeft: 4 }}>→ {job.target}</span>}
                        {job.progress !== null && (
                          <span style={{ color: CY, marginLeft: 4, fontSize: 8 }}>{job.progress}%</span>
                        )}
                        <span style={{ color: DIM, marginLeft: 6 }}>hits:{job.hits}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {expanded === node.id && !node.automated && (
                  <div style={{ marginLeft: 12, marginBottom: 6, fontSize: 9, color: DIM }}>
                    No swarm jobs target this graph node.
