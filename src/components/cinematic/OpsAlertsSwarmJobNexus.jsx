/**
 * OpsAlertsSwarmJobNexus — F648
 * "JARVIS, oalswrm / ops alert swarm / alert automation /
 *  swarm-backed alerts / which alerts have swarm /
 *  automated ops alerts / alert swarm coverage / ops automation coverage"
 * Cross-references /v1/ops/alerts against /entities/SwarmJob.
 * AUTOMATED alerts (≥1 swarm job keyword-matches) vs UNAUTOMATED (no swarm backing).
...
const BTN_LEFT = 110_540;
const Z_INDEX  = 187;

const OALSWRM_RE =
  /\boalswrm\b|\bops.?alert.?swarm\b|\balert.?automation\b|\bswarm.?backed.?alerts?\b|\bwhich.?alerts?.have.?swarm\b|\bautomated.?ops.?alerts?\b|\balert.?swarm.?coverage\b|\bops.?automation.?coverage\b|\bswarm.?alerts?\b|\balert.?swarm\b/i;

export function isOalswrmQuery(text) {
  return OALSWRM_RE.test(text || "");
}
...
function normaliseJobs(data) {
  ...
  return arr.map((j, i) => ({
    id:       j.id       || `job-${i}`,
    title:    j.title    || j.name    || j.job_type || `Job ${i + 1}`,
...
function crossRef(alerts, jobs) {
  return alerts.map((alert) => {
    const haystack = `${alert.title} ${alert.source} ${alert.message}`;
    const matches = jobs
      .map((job) => {
        const needle = `${job.title} ${job.description}`;
        const hits = overlap(haystack, needle);
        return hits > 0 ? { ...job, hits } : null;
      })
      .filter(Boolean)
      .sort((a, b) => b.hits - a.hits);
    return { ...alert, automated: matches.length > 0, jobs: matches };
  });
}

export async function buildOalswrmScript() {
  try {
    const base = apiBase();
    ...
    const [alertData, jobData] = await Promise.all([alertRes.json(), jobRes.json()]);
    const alerts = normaliseAlerts(alertData);
    const jobs   = normaliseJobs(jobData);
    const rows   = crossRef(alerts, jobs);
    ...
    const topUnautomated = rows
      ...
      .join("; ");
    return (
      `${automated} of ${rows.length} ops alerts are backed by active swarm jobs (${pct}% automation coverage). ` +
      (unautomated > 0
        ? `${unautomated} alert${unautomated !== 1 ? "s" : ""} lack swarm automation — manual response required for: ${topUnautomated || "unidentified alerts"}.`
        : "All active ops alerts are covered by at least one swarm job — full automation confirmed.")
    );
  } catch {
    return "Unable to reach ops alerts or swarm job endpoints, sir.";
  }
}
...
export default function OpsAlertsSwarmJobNexus() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:oalswrm-toggle", handler);
    return () => window.removeEventListener("jarvis:oalswrm-toggle", handler);
  }, [load, rows.length]);
  ...
  const visible = rows
    .filter((r) => {
      if (filter === "AUTOMATED")   return r.automated;
      if (filter === "UNAUTOMATED") return !r.automated;
      return true;
    })
    .filter((r) =>
      !search ||
  ...
  const assess = async () => {
    ...
    setBrief("");
    try {
      const summary = await buildOalswrmScript();
      const r = await fetch(`${apiBase()}/v1/jarvis/agent/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${API_KEY}` },
        body: JSON.stringify({ message: `JARVIS ops-alert swarm automation coverage brief: ${summary}` }),
      });
    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          bottom:   8,
          zIndex:   Z_INDEX,
          background: unautomated > 0 ? `${AMB}22` : "rgba(0,0,0,0.55)",
          border:   `1px solid ${unautomated > 0 ? AMB : CY}55`,
          borderRadius: 5,
          color:    unautomated > 0 ? AMB : CY,
          padding:  "3px 8px",
          fontSize: 9,
          letterSpacing: 1,
          cursor:   "pointer",
      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2 }}>OPS ALERTS × SWARM JOBS</span>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            {[
              ...
              { label: "AUTOMATED",   value: automated,   col: GRN },
              { label: "UNAUTOMATED", value: unautomated, col: AMB },
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
            {["ALL", "AUTOMATED", "UNAUTOMATED"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            {visible.map((alert) => (
              <div
                ...
                style={{
                  background: alert.automated ? `${GRN}06` : `${AMB}09`,
                  border: `1px solid ${alert.automated ? GRN + "22" : AMB + "33"}`,
                  borderRadius: 5,
                  padding: "6px 8px",
                  cursor: "pointer",
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{
                    fontSize: 8,
                    border: `1px solid ${alert.automated ? GRN + "44" : AMB + "44"}`,
                    borderRadius: 3,
                    padding: "1px 4px",
                    color: alert.automated ? GRN : AMB,
                    letterSpacing: 1,
                  }}>
                    {alert.automated ? "AUTOMATED" : "UNAUTOMATED"}
                  </span>
                  <span style={{ color: "#DCEBF5", fontSize: 10, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{alert.title}</span>
                  {alert.automated && (
                    <span style={{ color: DIM, fontSize: 9 }}>{alert.jobs.length} job{alert.jobs.length !== 1 ? "s" : ""}</span>
                  )}
                </div>
                {alert.source && (
                  ...
                  <div style={{ marginTop: 6, borderTop: `1px solid ${AMB}22`, paddingTop: 6 }}>
                    {alert.automated ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {alert.jobs.slice(0, 4).map((job) => (
                          <div key={job.id} style={{ background: `${GRN}08`, border: `1px solid ${GRN}22`, borderRadius: 4, padding: "5px 7px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                              <span style={{
                                color: STATUS_COLOR[job.status] || CY,
                                fontSize: 9,
                                border: `1px solid ${(STATUS_COLOR[job.status] || CY)}33`,
                                borderRadius: 3,
                                padding: "1px 4px",
                              }}>
                                {job.status}
                              </span>
                              <span style={{ color: "#DCEBF5", fontSize: 10, flex: 1 }}>{job.title}</span>
                              <span style={{ color: DIM, fontSize: 9 }}>hits: {job.hits}</span>
                            </div>
                            {job.progress !== null && (
                              <div style={{ marginTop: 4, background: `${GRN}22`, borderRadius: 3, height: 4, overflow: "hidden" }}>
                                <div style={{ width: `${Math.min(100, job.progress)}%`, height: "100%", background: GRN }} />
                              </div>
                            )}
                          </div>
                        ))}
                      ...
                      <div style={{ color: DIM, fontSize: 10 }}>No swarm jobs matched this alert — manual response or new swarm job required.</div>
                    )}
                  </div>
                )}
              </div>
