...
const IPASKDS_RE = /\b(ipaskds|intel\s+profile\s+skill|intel\s+aip\s+skill|intel\s+profile\s+dataset|unarmed\s+intel|intel\s+fully\s+armed|intel\s+profile\s+aip|intel\s+profile\s+coverage|intel\s+skill\s+dataset|intel\s+data\s+skill|profile\s+aip\s+dataset|intel\s+profile\s+triple)\b/i;

export function isIpaskdsQuery(t) { return IPASKDS_RE.test(t || ''); }

function kw(obj) {
  return [obj?.name, obj?.title, obj?.description, obj?.company, obj?.role,
          obj?.sector, obj?.nationality, obj?.aliases, obj?.tags, obj?.category, obj?.kind]
...
export async function buildIpaskdsScript() {
  const key = localStorage.getItem('jarvis_api_key') || 'dev-key';
  const hdr = { Authorization: `Bearer ${key}` };
  const [profR, skillR, dataR] = await Promise.allSettled([
  ...
  const datasets = (dataR.status  === 'fulfilled' ? (dataR.value?.data  ?? dataR.value  ?? []) : []).slice(0, 80);

  let fullyArmed = 0, skillBacked = 0, dataBacked = 0, unarmed = 0;
  for (const p of profiles) {
    const words = kw(p).split(/\s+/).filter(w => w.length > 3);
    const hasSkill = skills.some(s  => score(kw(s),  words) > 0.15);
    const hasData  = datasets.some(d => score(kw(d), words) > 0.15);
    if (hasSkill && hasData) fullyArmed++;
    else if (hasSkill) skillBacked++;
    else if (hasData) dataBacked++;
    else unarmed++;
  }

  return `IPASKDS: ${profiles.length} intel profiles × ${skills.length} AIP skills × ${datasets.length} datasets. ` +
    `${fullyArmed} FULLY ARMED, ${skillBacked} SKILL-BACKED, ${dataBacked} DATA-BACKED, ${unarmed} UNARMED. ` +
    (unarmed > 0
      ? `${unarmed} intel profiles lack both AIP skill tools and dataset evidence — intelligence capability gap.`
      : 'All intel profiles have AIP skill or dataset coverage.');
}

const GR = '#00FF88'; const VL = '#A855F7'; const EM = '#10B981'; const RD = '#EF4444';
const BG = 'rgba(6,16,28,0.97)'; const BD = 'rgba(0,212,255,0.18)'; const CY = '#00D4FF';

export default function IntelProfileAipDatasetTriple() {
  ...
  useEffect(() => {
    const h = () => setOpen(o => !o);
    window.addEventListener('jarvis:ipaskds-toggle', h);
    return () => window.removeEventListener('jarvis:ipaskds-toggle', h);
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const key = localStorage.getItem('jarvis_api_key') || 'dev-key';
      const hdr = { Authorization: `Bearer ${key}` };
      const [pR, sR, dR] = await Promise.allSettled([
        fetch(`${API}/entities/IntelProfile`, { headers: hdr }).then(r => r.json()),
        fetch(`${API}/v1/aip/skill`,          { headers: hdr }).then(r => r.json()),
        fetch(`${API}/v1/datasets`,           { headers: hdr }).then(r => r.json()),
      ]);
      const profs = (pR.status === 'fulfilled' ? (pR.value?.data ?? pR.value ?? []) : []).slice(0, 80);
      ...
      setProfiles(profs); setSkills(skls); setDatasets(dats);
      ...
      const built = profs.map(p => {
        ...
        const state = hasS && hasD ? 'FULLY ARMED'
          : hasS ? 'SKILL-BACKED'
          : hasD ? 'DATA-BACKED'
          : 'UNARMED';
        return { ...p, matchedSkills, matchedDatasets, state };
      });
      setRows(built);
  ...
  const unarmed     = rows.filter(r => r.state === 'UNARMED').length;
  ...
  const assess = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({ message: `Summarise intel profile AIP skill and dataset coverage in 2 sentences: ${fullyArmed} fully armed (skill+dataset), ${skillBacked} skill-only, ${dataBacked} data-only, ${unarmed} unarmed out of ${rows.length} intel profiles.` }),
      });
  ...
  return (
    <div style={{ position: 'fixed', left: 800560, bottom: 8, zIndex: 458, width: 560,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px',
        ...
        <span style={{ color: CY, fontWeight: 700, letterSpacing: 2, fontSize: 10 }}>◈ IPASKDS</span>
        <span style={{ fontSize: 9, color: '#6E8AA0' }}>INTEL-PROFILE × AIP-SKILL × DATASET</span>
        {unarmed > 0 && (
          <span style={{ marginLeft: 'auto', background: RD, color: '#fff', borderRadius: 4,
            padding: '1px 6px', fontSize: 9, fontWeight: 700 }}>{unarmed} UNARMED</span>
        )}
        <button onClick={() => setOpen(false)} style={{ marginLeft: unarmed > 0 ? 4 : 'auto',
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 12px', flexShrink: 0 }}>
        {[['PROFILES', profiles.length, CY], ['AIP SKILLS', skills.length, '#6E8AA0'],
          ['DATASETS', datasets.length, '#6E8AA0'], ['FULLY ARMED', fullyArmed, GR],
          ['SKILL-BKD', skillBacked, VL], ['DATA-BKD', dataBacked, EM], ['UNARMED', unarmed, RD]
        ].map(([lbl, val, col]) => (
      ...
      <div style={{ display: 'flex', height: 4, margin: '0 12px 8px', borderRadius: 2, overflow: 'hidden', flexShrink: 0 }}>
        ...
        <div style={{ width: `${(unarmed     / total) * 100}%`, background: RD }} />
      </div>

      {/* filter tabs */}
      <div style={{ display: 'flex', gap: 4, padding: '0 12px 6px', flexShrink: 0, flexWrap: 'wrap' }}>
        {['ALL', 'FULLY ARMED', 'SKILL-BACKED', 'DATA-BACKED', 'UNARMED'].map(f => (
          ...
          return (
            <div key={id} style={{ borderBottom: `1px solid rgba(0,212,255,0.07)` }}>
              <div onClick={() => setExpanded(isExp ? null : id)}
                ...
                <span style={{ color: stateColor(r.state), fontSize: 9, minWidth: 100,
                  fontWeight: 700, letterSpacing: 1 }}>{r.state}</span>
                <span style={{ flex: 1, color: '#DCEBF5', overflow: 'hidden',
                  textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {r.name || r.title || r.id || '—'}
                </span>
                {r.role && <span style={{ color: '#4A6080', fontSize: 9 }}>{r.role}</span>}
