...
const API = '';
const DSAL_RE = /\b(dataset[._-]?alert|alert[._-]?dataset|dsal|datasets?[._-]?with[._-]?alert|exposed[._-]?dataset|dataset[._-]?alert[._-]?correlat|which[._-]?datasets?[._-]?have[._-]?alert)\b/i;

export function isDsalQuery(t) {
  return DSAL_RE.test(t || '');
}

export async function buildDsalScript() {
  const [dsR, alR] = await Promise.allSettled([
  ...
  const enriched = correlate(datasets, alerts);
  const exposed = enriched.filter(d => d._linked).length;
  const clear = enriched.length - exposed;
  return (
    `Dataset × Alert Correlation: ${datasets.length} datasets, ${alerts.length} active alerts indexed. ` +
    `${exposed} datasets are matched to active alerts; ${clear} appear clear. ` +
...
function matchScore(dataset, alert) {
  const dToks = new Set([
    ...tokens(dataset.name),
    ...tokens(dataset.key),
...
function correlate(datasets, alerts) {
  return datasets.map(dataset => {
    const scored = alerts
      .map(alert => ({ alert, score: matchScore(dataset, alert) }))
      .filter(s => s.score > 0)
...
export default function DatasetAlertCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:dsal-toggle', onToggle);
    return () => window.removeEventListener('jarvis:dsal-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${datasets.length} datasets and ${alerts.length} active alerts. ${exposed.length} datasets are matched to active alerts; ${clear.length} appear clear. Give a 2-sentence dataset alert coverage brief identifying the key risk pattern and which datasets or categories are most alert-correlated.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Dataset × Alert Correlator (DSAL)"
        style={{
          position: 'fixed', left: 729760, bottom: 8, zIndex: 254,
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {dataset._matches.length > 0 ? (
                        <>
                          ...
                          {dataset._matches.map(({ alert, score }, j) => {
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No alerts matched this dataset.</div>
                      )}
                    </div>
                  )}
                </div>
