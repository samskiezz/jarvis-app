/**
 * ContactKnowledgeBridge — F82 (CKIB)
 *
 * Parallel-fetches /entities/Contact + /knowledge/ every 90 s.
 * Keyword-correlates each contact (name/email/organization/tags) against
 * KB articles (title/content/tags/summary).
 * Classification: DOCUMENTED (≥1 article match) vs UNKNOWN (0 matches).
 * Amber badge on unknown count.
 *
 * Voice intents: "contact knowledge / knowledge contact / ckib /
 *                documented contacts / unknown contacts knowledge /
 *                kb contact / contact kb / who has knowledge /
 *                contact knowledge bridge / contact documentation"
 * Strip button: ◈ CKIB  left:2400 bottom:18 zIndex:68
 * Custom event: jarvis:ckib-toggle
...
const CKIB_RE =
  /\b(contact.knowledge|knowledge.contact|ckib|documented.contact|unknown.contact.knowledge|kb.contact|contact.kb|who.has.knowledge|contact.knowledge.bridge|contact.documentation|contact.kb.coverage|knowledge.coverage.contact)\b/i;

export function isCkibQuery(t) { return CKIB_RE.test(t || ""); }
...
export async function buildCkibScript() {
  try {
    const base = apiBase();
    const { contacts, articles } = await fetchAll();
    const threshold = 0.04;
    const documented = contacts.filter(c => articles.some(a => relevance(c, a) >= threshold));
    const unknown    = contacts.filter(c => !articles.some(a => relevance(c, a) >= threshold));
    const r = await fetch(`${base}/v1/jarvis/agent/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...hdrs },
      body: JSON.stringify({
        message: `Contact × Knowledge Intelligence Bridge (CKIB): ${contacts.length} contacts, ` +
          `${articles.length} KB articles, ${documented.length} documented, ` +
          `${unknown.length} unknown (no KB match). Unknown contacts: ` +
          `${unknown.slice(0, 3).map(c => c.name).join("; ") || "none"}. ` +
          "Assess knowledge coverage gaps for contacts in exactly 2 sentences.",
      }),
    });
    const d = r.ok ? await r.json() : null;
    return d?.response || d?.reply || d?.content || d?.text ||
      `CKIB: ${documented.length}/${contacts.length} contacts documented in KB, ${unknown.length} unknown.`;
  } catch {
    return "CKIB: unable to fetch assessment.";
  }
}
...
/* ── Component ─────────────────────────────────────────────────── */
export default function ContactKnowledgeBridge() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const enriched = contacts.map(c => {
        ...
        return { ...c, status: matches.length > 0 ? "DOCUMENTED" : "UNKNOWN", matches };
      });
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:ckib-toggle", toggle);
    return () => window.removeEventListener("jarvis:ckib-toggle", toggle);
  }, []);
  ...
  const runAssess = async () => {
    ...
    const txt = await buildCkibScript();
    setAssess(txt); setAssessing(false);
    window.dispatchEvent(new CustomEvent("jarvis:speak-dossier", { detail: { text: txt } }));
  };

  if (!open) {
    const unknownCount = data ? data.contacts.filter(c => c.status === "UNKNOWN").length : 0;
    return (
      <button style={BTN} onClick={() => setOpen(true)} title="Contact × Knowledge Bridge">
        ◈ CKIB
        {unknownCount > 0 && (
  ...
  const unknown    = contacts.filter(c => c.status === "UNKNOWN");

  const visible = contacts.filter(c => {
    if (filter === "DOCUMENTED" && c.status !== "DOCUMENTED") return false;
    if (filter === "UNKNOWN"    && c.status !== "UNKNOWN")    return false;
  ...
  return (
    <div style={PANEL}>
      ...
      <div style={{ padding: "10px 14px 6px", borderBottom: "1px solid #29E7FF22", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ color: CY, fontWeight: 700, fontSize: 12, letterSpacing: 1 }}>◈ CONTACT × KNOWLEDGE BRIDGE</span>
      ...
      <div style={{ display: "flex", gap: 6, padding: "8px 14px", borderBottom: "1px solid #29E7FF11" }}>
        {[
          ...
          { label: "ARTICLES", val: articles.length, color: CY },
          { label: "DOCUMENTED", val: documented.length, color: GRN },
          { label: "UNKNOWN",    val: unknown.length,    color: AMB },
        ].map(t => (
