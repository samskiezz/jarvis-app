...
const API = '';
const IPDS_RE = /\b(intel[._-]?profile[._-]?dataset|dataset[._-]?intel|ipds|intel[._-]?data[._-]?cov|data[._-]?backed[._-]?intel|intel[._-]?dataset[._-]?corr|which[._-]?intel[._-]?has[._-]?dataset|intel[._-]?profile[._-]?data[._-]?cov|dataset[._-]?intel[._-]?cov)\b/i;

export function isIpdsQuery(t) {
  return IPDS_RE.test(t || '');
}

export async function buildIpdsScript() {
  const [intelR, dsR] = await Promise.allSettled([
  ...
  const enriched = correlate(profiles, datasets);
  const documented = enriched.filter(p => p._linked).length;
  const blind = enriched.length - documented;
  return (
    `IntelProfile × Dataset Coverage: ${profiles.length} intel profiles, ${datasets.length} datasets indexed. ` +
    `${documented} profiles have dataset backing; ${blind} have no data coverage. ` +
    `Blind: ${enriched.filter(p => !p._linked).slice(0, 4).map(p => p.name || p.subject || p.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(profile, dataset) {
  ...
  if (!profToks.size || !dsToks.length) return 0;
  let hits = 0;
  for (const t of dsToks) if (profToks.has(t)) hits++;
  return hits / Math.max(profToks.size, dsToks.length);
}

function correlate(profiles, datasets) {
  return profiles.map(profile => {
    const scored = datasets
      .map(ds => ({ ds, score: matchScore(profile, ds) }))
      .filter(s => s.score > 0)
...
export default function IntelProfileDatasetCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:ipds-toggle', onToggle);
    return () => window.removeEventListener('jarvis:ipds-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(profiles, datasets);
  const documented = enriched.filter(p => p._linked);
  const blind = enriched.filter(p => !p._linked);
  const badgeCount = blind.length;
  const badgeColor = badgeCount > 0 ? AM : GR;

  const filtered = enriched
    .filter(p => tab === 'ALL' || (tab === 'DOCUMENTED' ? p._linked : !p._linked))
    .filter(p => {
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${profiles.length} intel profiles and ${datasets.length} datasets. ${documented.length} profiles have dataset backing; ${blind.length} have no data coverage. Give a 2-sentence intel-data coverage brief highlighting which intelligence subjects lack dataset support.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="IntelProfile × Dataset Coverage (IPDS)"
        style={{
          position: 'fixed', left: 675600, bottom: 8, zIndex: 242,
      ...
      {open && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              { label: 'INTEL', val: profiles.length, col: CY },
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {profile.category && chip(profile.category, VI)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {profile._matches.length > 0 ? (
                        <>
                          ...
                          {profile._matches.map(({ ds, score }, j) => (
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No datasets matched this intel profile — data coverage gap.</div>
                      )}
                    </div>
                  )}
                </div>
