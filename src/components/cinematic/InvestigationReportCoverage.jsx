...
const API = '';
const IRPC_RE = /\b(investigation[._-]?report|report[._-]?investigation|irpc|cases?[._-]?with[._-]?reports?|covered[._-]?investigations?|undocumented[._-]?cases?|investigation[._-]?report[._-]?coverage|which[._-]?investigations?[._-]?have[._-]?reports?|case[._-]?documentation[._-]?coverage)\b/i;

export function isIrpcQuery(t) {
  return IRPC_RE.test(t || '');
}

export async function buildIrpcScript() {
  const [invR, rpR] = await Promise.allSettled([
    fetch(`${API}/v1/investigations`).then(r => r.json()),
    fetch(`${API}/v1/reports`).then(r => r.json()),
  ]);
  const cases = normaliseInvestigations(invR.status === 'fulfilled' ? invR.value : []);
  const reports = normaliseReports(rpR.status === 'fulfilled' ? rpR.value : []);
  const enriched = correlate(cases, reports);
  const covered = enriched.filter(c => c._linked).length;
  const undocumented = enriched.length - covered;
  return (
    `Investigation × Report Coverage: ${cases.length} investigations, ${reports.length} reports indexed. ` +
    `${covered} cases are backed by a report; ${undocumented} have no report documentation. ` +
    `Top undocumented: ${enriched.filter(c => !c._linked).slice(0, 4).map(c => c.title || c.name || c.id || '?').join(', ') || 'none'}.`
  );
}
...
function matchScore(inv, report) {
  const iToks = new Set([
    ...tokens(inv.title),
    ...tokens(inv.name),
...
function correlate(cases, reports) {
  return cases.map(inv => {
    const scored = reports
      .map(report => ({ report, score: matchScore(inv, report) }))
      .filter(s => s.score > 0)
...
export default function InvestigationReportCoverage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCases(normaliseInvestigations(invR.status === 'fulfilled' ? invR.value : []));
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:irpc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:irpc-toggle', onToggle);
  }, []);
  ...
  const enriched = correlate(cases, reports);
  const covered = enriched.filter(c => c._linked);
  const undocumented = enriched.filter(c => !c._linked);
  const badgeCount = undocumented.length;
  const badgeColor = badgeCount > 0 ? AM : GR;

  const filtered = enriched
    .filter(c => tab === 'ALL' || (tab === 'COVERED' ? c._linked : !c._linked))
    .filter(c => {
      ...
      return (
        ...
        String(c.kind || '').toLowerCase().includes(q) ||
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${cases.length} investigations and ${reports.length} reports. ${covered.length} cases are backed by a report; ${undocumented.length} have no report documentation. Give a 2-sentence case-documentation brief identifying the key intelligence gap and which undocumented investigation types most need report backing.`,
        }),
      });
  ...
  const caseLabel = c => c.title || c.name || c.subject || c.id || '?';

  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Investigation × Report Coverage (IRPC)"
        style={{
          position: 'fixed', left: 802720, bottom: 8, zIndex: 270,
      ...
      {open && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, padding: '8px 14px', flexShrink: 0 }}>
            {[
              { label: 'CASES', val: cases.length, col: CY },
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: '#DCEBF5', fontSize: 11, flex: 1 }}>{caseLabel(inv)}</span>
                    {inv.kind && chip(inv.kind, CY)}
                    {inv.status && chip(inv.status, inv.status === 'open' ? GR : AM)}
                    {chip(inv._linked ? 'COVERED' : 'UNDOCUMENTED', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {inv._matches.length > 0 ? (
                        <>
                          ...
                          {inv._matches.map(({ report, score }, j) => (
                            <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                              {report.category && chip(report.category, PR)}
                        ...
                        <div style={{ color: AM, fontSize: 10 }}>No reports matched this investigation.</div>
                      )}
                    </div>
                  )}
                </div>
