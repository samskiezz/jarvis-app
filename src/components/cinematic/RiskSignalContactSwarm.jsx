/**
 * F186 — RiskSignal × Contact × SwarmJob — Threat Response Ownership Index (TROI)
 *
 * Parallel-fetches /entities/RiskSignal + /entities/Contact + /entities/SwarmJob every 90 s.
 * Keyword-correlates each risk signal against responsible contacts AND active swarm jobs:
 *
 *   FULLY_OWNED   — matched ≥1 contact AND ≥1 swarm job
 *   CONTACT_ONLY  — contact assigned, no swarm job working it
 *   SWARM_ONLY    — swarm job active, no contact owner
 *   UNOWNED       — no contact and no swarm job coverage
 *
 * Stat tiles: signals / contacts / swarm jobs / fully owned / unowned
 * Filter tabs: ALL | FULLY_OWNED | CONTACT_ONLY | SWARM_ONLY | UNOWNED
 * Text search on signal name / severity / description.
 * Expand row → matched contacts (cyan bars) + matched swarm jobs (amber bars).
 * Red badge + pulse on UNOWNED count.
 * ▶ ASSESS → /v1/jarvis/agent/chat 2-sentence brief + TTS.
 *
 * Toggle:  ◈ TROI  at bottom:8 left:979400, zIndex:687.
 * Event:   jarvis:troi-toggle
 * Voice:   "troi / threat ownership / risk ownership / unowned risk /
 *           threat response ownership / risk signal contact / risk swarm coverage"
 * Refresh: 90 s auto-poll.
 */
import { useCallback, useEffect, useRef, useState } from "react";

const BTN_LEFT = 979_400;
...
const TROI_RE =
  /\b(troi|threat\s+ownership|risk\s+ownership|unowned\s+risk|threat\s+response\s+ownership|risk\s+signal\s+contact|risk\s+swarm\s+coverage)\b/i;

export function isTroiQuery(q) {
  return TROI_RE.test(q || "");
}

export async function buildTroiScript() {
  const base = apiBase();
  const h = { Authorization: `Bearer ${API_KEY}` };
  try {
    const [rsr, cr, sjr] = await Promise.allSettled([
      ...
      fetch(`${base}/entities/SwarmJob?limit=200`,   { headers: h }).then(r => r.ok ? r.json() : []),
    ]);
    const signals  = toArr(rsr.value);
    const contacts = toArr(cr.value);
    const swarms   = toArr(sjr.value);
    let unowned = 0;
    signals.forEach(sig => {
      const kws = keywords(sig);
      const hasContact = contacts.some(c => matchKws(kws, c));
      const hasSwarm   = swarms.some(s => matchKws(kws, s));
      if (!hasContact && !hasSwarm) unowned++;
    });
    return `Threat Response Ownership: ${signals.length} risk signals assessed against ${contacts.length} contacts and ${swarms.length} swarm jobs. ${unowned} signals have no assigned contact and no active swarm coverage — these represent unowned threat exposure requiring immediate triage.`;
  } catch {
    return "TROI assessment unavailable.";
  }
}
...
function toArr(v) {
  ...
  if (Array.isArray(v.signals)) return v.signals;
...
function txt(obj) {
  return [
    ...
    obj?.category, obj?.entity_type, obj?.severity,
    obj?.signal_type, obj?.threat_type,
...
function classify(sig, contacts, swarms) {
  const kws       = keywords(sig);
  const hasContact = contacts.some(c => matchKws(kws, c));
  const hasSwarm   = swarms.some(s => matchKws(kws, s));
  if (hasContact && hasSwarm)  return "FULLY_OWNED";
...
const CLASS_ORDER = ["FULLY_OWNED", "CONTACT_ONLY", "SWARM_ONLY", "UNOWNED"];

// ── Component ─────────────────────────────────────────────────────────────────

export default function RiskSignalContactSwarm() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setRows(signals.map(sig => ({
        sig,
        cls: classify(sig, contacts, swarms),
        matchedContacts: contacts.filter(c => matchKws(keywords(sig), c)).slice(0, 5),
        matchedSwarms:   swarms.filter(s => matchKws(keywords(sig), s)).slice(0, 5),
      })));
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    const onAsk = (e) => {
      ...
      if (isTroiQuery(q)) setOpen(true);
    };
    window.addEventListener("jarvis:troi-toggle", onToggle);
    window.addEventListener("jarvis:ask", onAsk);
    return () => {
      window.removeEventListener("jarvis:troi-toggle", onToggle);
  ...
  async function assess() {
    ...
    setAssessment("");
    try {
      const script = await buildTroiScript();
      ...
      const fallback = await buildTroiScript();
      setAssessment(fallback);
    }
    setAssessing(false);
  }

  const unownedCount = rows.filter(r => r.cls === "UNOWNED").length;
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(v => !v)}
        title="RiskSignal × Contact × SwarmJob — Threat Response Ownership (TROI)"
        style={{
          position: "fixed", left: BTN_LEFT, bottom: 8, zIndex: 687,
          background: open ? RED + "cc" : "rgba(5,8,13,0.78)",
          border: `1px solid ${unownedCount > 0 ? RED : "#334F62"}88`,
          borderRadius: 8, padding: "3px 8px", cursor: "pointer",
          color: open ? "#fff" : unownedCount > 0 ? RED : CY,
          fontFamily: MONO, fontSize: 9, letterSpacing: 1,
          boxShadow: unownedCount > 0 ? `0 0 10px ${RED}44` : "none",
          animation: unownedCount > 0 ? "troi-pulse 2s infinite" : "none",
        }}
      >
        ◈ TROI{unownedCount > 0 ? ` ${unownedCount}!` : ""}
      </button>
      <style>{`
        @keyframes troi-pulse {
      ...
      {open && (
        <div style={{
          position: "fixed", left: Math.max(8, BTN_LEFT - 340), bottom: 36, zIndex: 687,
          ...
          <div style={{
            ...
            <span style={{ color: CY, fontSize: 10, letterSpacing: 2 }}>◈ TROI</span>
            <span style={{ color: "#334F62", fontSize: 9 }}>threat response ownership</span>
          ...
          <div style={{
            ...
            {[
              ["SIGNALS",   rows.length,     CY],
              ["CONTACTS",  contactCount,    GREEN],
              ["SWARM JOBS",swarmCount,      AMBER],
              ["OWNED",     stats.FULLY_OWNED, CY],
              ["UNOWNED",   unownedCount,    RED],
            ].map(([label, val, col]) => (
              <div key={label} style={{
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {displayRows.length === 0 ? (
              ...
              const { sig, cls, matchedContacts, matchedSwarms } = row;
              const col      = CLASS_COLORS[cls];
              const label    = sig?.name || sig?.title || sig?.label || sig?.id || `Signal ${i + 1}`;
              const severity = sig?.severity || sig?.level || sig?.priority || "";
                ...
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    <span style={{
                      ...
                      boxShadow: cls === "UNOWNED" ? `0 0 6px ${RED}` : "none",
                    }} />
                    <span style={{
                    ...
                    {severity && (
                      <span style={{ color: "#334F62", fontSize: 8, flexShrink: 0 }}>
                        {String(severity).slice(0, 8).toUpperCase()}
                      </span>
                    )}
                    ...
                    <div style={{ padding: "4px 12px 8px" }}>
                      ...
                      <div>
                        <div style={{ color: AMBER, fontSize: 8, marginBottom: 3 }}>
                          SWARM JOBS ({matchedSwarms.length})
                        </div>
                        {matchedSwarms.length === 0
                          ? <div style={{ color: "#334F62", fontSize: 9 }}>none</div>
                          : matchedSwarms.map((s, si) => (
                            <div key={si} style={{ marginBottom: 3 }}>
                              ...
                              <div style={{ color: "#5A7A90", fontSize: 8, marginTop: 1 }}>
                                {(s?.name || s?.title || s?.id || "SwarmJob").slice(0, 50)}
                              </div>
                            </div>
                          ))
                        }
          ...
          <div style={{ padding: "4px 12px 6px", color: "#334F62", fontSize: 9 }}>
            auto-refresh {POLL_MS / 1000}s · {rows.length} signals · {contactCount} contacts · {swarmCount} swarm jobs
          </div>
        </div>
      )}
    </>
