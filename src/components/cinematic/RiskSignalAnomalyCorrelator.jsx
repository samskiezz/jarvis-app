...
const API = '';
const RSAC_RE = /\b(risk[._-]?signal[._-]?anom|anom[._-]?risk[._-]?signal|rsac|exposed[._-]?risk[._-]?signals?|risk[._-]?signals?[._-]?with[._-]?anom|which[._-]?risk[._-]?signals?[._-]?have[._-]?anom|risk[._-]?metric[._-]?anom|risk[._-]?anom[._-]?correlat|risk[._-]?anomaly[._-]?correlat|anomaly[._-]?risk[._-]?signal)\b/i;

export function isRsacQuery(t) {
  return RSAC_RE.test(t || '');
}

export async function buildRsacScript() {
  const [rsR, anomR] = await Promise.allSettled([
  ...
  const enriched = correlate(signals, anomalies);
  const exposed = enriched.filter(s => s._linked).length;
  const clear = enriched.length - exposed;
  return (
    `Risk Signal × Anomaly Correlation: ${signals.length} risk signals, ${anomalies.length} metric anomalies indexed. ` +
    `${exposed} signals are exposed by active metric anomalies; ${clear} appear clear. ` +
...
function matchScore(signal, anomaly) {
  ...
  if (!sigToks.size || !anomToks.length) return 0;
  let hits = 0;
  for (const t of anomToks) if (sigToks.has(t)) hits++;
  return hits / Math.max(sigToks.size, anomToks.length);
}

function correlate(signals, anomalies) {
  return signals.map(signal => {
    const scored = anomalies
      .map(anom => ({ anom, score: matchScore(signal, anom) }))
      .filter(s => s.score > 0)
...
export default function RiskSignalAnomalyCorrelator() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSignals(normaliseArray(rsR.status === 'fulfilled' ? rsR.value : []));
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:rsac-toggle', onToggle);
    return () => window.removeEventListener('jarvis:rsac-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${signals.length} risk signals and ${anomalies.length} metric anomalies. ${exposed.length} risk signals are exposed by active anomalies; ${clear.length} appear clear. Give a 2-sentence risk anomaly correlation brief with the key risk pattern.`,
        }),
      });
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        title="Risk Signal × Anomaly Correlator (RSAC)"
        style={{
          position: 'fixed', left: 630000, bottom: 8, zIndex: 231,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2, fontWeight: 700, textShadow: `0 0 12px ${CY}` }}>
              ◈ RISK SIGNAL × ANOMALY CORRELATOR
            </span>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {signal.sector && chip(signal.sector, '#6E8AA0')}
                    {chip(signal._linked ? 'EXPOSED' : 'CLEAR', statusColor)}
                    ...
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {signal._matches.length > 0 ? (
                        <>
                          ...
                          {signal._matches.map(({ anom, score }, j) => {
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No anomalies matched this risk signal.</div>
                      )}
                    </div>
                  )}
                </div>
