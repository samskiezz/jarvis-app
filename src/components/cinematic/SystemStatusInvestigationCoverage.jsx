...
const SSIVCO_RE = /\b(ssivco|system\s+invest|investigation\s+system|system\s+monitored\s+investigation|untracked\s+investigation\s+system|investigation\s+service\s+coverage|service\s+investigation\s+coverage|system\s+status\s+invest)\b/i;

export function isSsivcoQuery(t) { return SSIVCO_RE.test(t || ''); }

export async function buildSsivcoScript() {
  const key = localStorage.getItem('jarvis_api_key') || 'dev-key';
  const hdr = { Authorization: `Bearer ${key}` };
  const [stR, invR] = await Promise.allSettled([
  ...
  let monitored = 0, untracked = 0;
  for (const inv of investigations) {
    const text = `${inv.title ?? inv.name ?? ''} ${inv.description ?? ''} ${inv.subject ?? ''} ${inv.type ?? ''}`.toLowerCase();
    const tokens = text.split(/\W+/).filter(t => t.length > 2);
    const hit = tokens.some(tok => svcText.includes(tok));
    if (hit) monitored++; else untracked++;
  }
  return `SSIVCO System Status × Investigation Coverage: ${investigations.length} investigations assessed against ${services.length} live services. ` +
    `MONITORED: ${monitored} (system service backing found). ` +
    `UNTRACKED: ${untracked} (no system service monitors this investigation domain — operational gap).`;
}
...
const STATE_COLOR = { MONITORED: '#22d3ee', UNTRACKED: '#f59e0b' };
const STATE_ORDER = ['MONITORED', 'UNTRACKED'];

function scoreMatch(inv, services) {
  const text = `${inv.title ?? inv.name ?? ''} ${inv.description ?? ''} ${inv.subject ?? ''} ${inv.type ?? ''}`.toLowerCase();
  const tokens = text.split(/\W+/).filter(t => t.length > 2);
  const matched = [];
...
function correlate(inv, services) {
  const text = `${inv.title ?? inv.name ?? ''} ${inv.description ?? ''} ${inv.subject ?? ''} ${inv.type ?? ''}`.toLowerCase();
  const tokens = text.split(/\W+/).filter(t => t.length > 2);
  const svcText = services.map(s => `${s.name ?? s.id ?? s.service ?? ''}`.toLowerCase()).join(' ');
  const hit = tokens.some(tok => svcText.includes(tok));
  return hit ? 'MONITORED' : 'UNTRACKED';
}

export default function SystemStatusInvestigationCoverage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const svcs = Array.isArray(raw) ? raw
        : (raw.services ?? raw.components ?? raw.checks ?? raw.data ?? []);
      const invRaw = invR.value ?? {};
      const invs = Array.isArray(invRaw) ? invRaw
        : (invRaw.investigations ?? invRaw.data ?? invRaw.results ?? []);
      setServices(svcs);
      setInvestigations(invs);
      setRows(invs.map(inv => ({ ...inv, state: correlate(inv, svcs), matches: scoreMatch(inv, svcs) })));
  ...
  useEffect(() => {
    window.addEventListener('jarvis:ssivco-toggle', toggle);
    return () => window.removeEventListener('jarvis:ssivco-toggle', toggle);
  }, [toggle]);
  ...
  const assess = useCallback(async () => {
    ...
    const monitored = rows.filter(r => r.state === 'MONITORED').length;
    const summary = `SSIVCO: ${rows.length} investigations assessed. MONITORED: ${monitored}. UNTRACKED: ${untracked} (no system service monitors these investigation domains — operational gap).`;
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
        body: JSON.stringify({ message: `Assess this SSIVCO system-status × investigation coverage state in 2 sentences and recommend which untracked investigations need system service instrumentation: ${summary}` }),
      });
  ...
  const untrackedCount = stateCounts['UNTRACKED'] ?? 0;

  const PANEL = {
    position: 'fixed', right: 16, top: 16, zIndex: 9984, width: 660, maxHeight: 600,
  ...
  if (!open) {
    return (
      <button onClick={toggle} title="SSIVCO System Status × Investigation Coverage" style={{
        position: 'fixed', left: 780960, bottom: 8, zIndex: 423,
  ...
  const monitored = stateCounts['MONITORED'] ?? 0;
  ...
  return (
    <div style={PANEL}>
      <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ color: '#f59e0b', fontWeight: 700, fontSize: 11, letterSpacing: 2 }}>◈ SSIVCO</span>
        <span style={{ color: '#64748b', fontSize: 10, flex: 1 }}>System Status × Investigation Coverage</span>
      ...
      <div style={{ padding: '6px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: '#64748b', marginBottom: 4 }}>
          <span>SYSTEM MONITORING COVERAGE</span><span>{pct}%</span>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 14px' }}>
        {visible.length === 0 ? (
          ...
          const id = row.id ?? row._id ?? row.title ?? i;
          ...
          return (
            <div key={id} style={{ marginBottom: 4 }}>
              ...
              >
                ...
                {row.status && <span style={{ fontSize: 8, color: row.status === 'open' || row.status === 'active' ? '#22d3ee' : '#64748b' }}>{row.status}</span>}
              ...
              {isExp && (
                <div style={{ margin: '2px 0 4px 12px', padding: '8px 10px', background: 'rgba(0,0,0,0.3)', borderRadius: 4, borderLeft: `2px solid ${STATE_COLOR[row.state]}44` }}>
                  {row.description && <div style={{ fontSize: 9, color: '#94a3b8', marginBottom: 6 }}>{row.description.slice(0, 180)}{row.description.length > 180 ? '…' : ''}</div>}
                  {row.matches && row.matches.length > 0 ? (
                    ...
                    <div style={{ fontSize: 9, color: '#f59e0b' }}>⚠ No system service monitors this investigation domain</div>
                  )}
                </div>
              )}
            </div>
