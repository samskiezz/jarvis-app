...
const API = '';
const TRSC_RE = /\b(task[._-]?risk|risk[._-]?task|trsc|exposed[._-]?tasks?|tasks?[._-]?with[._-]?risks?|risky[._-]?tasks?|task[._-]?risk[._-]?signals?|tasks?[._-]?under[._-]?risk|which[._-]?tasks?[._-]?are[._-]?exposed|risk[._-]?exposed[._-]?tasks?|task[._-]?risk[._-]?coverage)\b/i;

export function isTrscQuery(t) {
  return TRSC_RE.test(t || '');
}

export async function buildTrscScript() {
  ...
  const enriched = correlate(tasks, signals);
  const exposed = enriched.filter(t => t._linked).length;
  const clear = enriched.filter(t => !t._linked).length;
  return (
    `Task × Risk Signal Correlation: ${tasks.length} tasks, ${signals.length} risk signals indexed. ` +
    `${exposed} tasks are exposed to active risk signals; ${clear} appear clear. ` +
...
function normaliseArray(raw) {
  ...
  for (const k of ['items', 'results', 'data', 'tasks', 'signals', 'entities', 'records']) {
...
function matchScore(task, signal) {
  ...
  if (!taskToks.size || !sigToks.length) return 0;
  let hits = 0;
  for (const t of sigToks) if (taskToks.has(t)) hits++;
  return hits / Math.max(taskToks.size, sigToks.length);
}

function correlate(tasks, signals) {
  return tasks.map(task => {
    const scored = signals
      .map(sig => ({ sig, score: matchScore(task, sig) }))
      .filter(s => s.score > 0)
...
export default function TaskRiskSignalCorrelator() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis:trsc-toggle', onToggle);
    return () => window.removeEventListener('jarvis:trsc-toggle', onToggle);
  }, []);
  ...
  async function assess() {
    ...
    try {
      const r = await fetch(`${API}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message: `You have ${tasks.length} tasks and ${signals.length} risk signals. ${exposed.length} tasks are exposed to risk signals; ${clear.length} appear clear. Give a 2-sentence task risk exposure brief with the key risk coverage gap.`,
        }),
      });
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        title="Task × Risk Signal Correlator (TRSC)"
        style={{
          position: 'fixed', left: 511440, bottom: 8, zIndex: 205,
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 14px 8px' }}>
            {filtered.length === 0 ? (
              ...
              const statusColor = task._linked ? RD : GR;
              return (
                <div
                  key={task.id || i}
                ...
                >
                  ...
                  >
                    ...
                    {task.type && chip(task.type, '#A78BFA')}
                    {task.status && chip(task.status, '#6E8AA0')}
                    {chip(task._linked ? 'EXPOSED' : 'CLEAR', statusColor)}
                  ...
                  {isExp && (
                    <div style={{ paddingLeft: 14, paddingTop: 4 }}>
                      {task._matches.length > 0 ? (
                        ...
                        <div style={{ color: GR, fontSize: 10 }}>No risk signals matched this task.</div>
                      )}
                    </div>
                  )}
                </div>
