/**
 * RemindersKnowledgeNexus — F601
 * "JARVIS, reminders knowledge / knowledge reminders / remkno / knowledge-backed reminders / floating notes / reminder articles"
 * Cross-references /reminders/list against /knowledge/ articles.
 * KNOWLEDGE-BACKED reminders (≥1 article keyword-matches) vs FLOATING (no knowledge backing).
 * Coverage % tile; ALL/KNOWLEDGE-BACKED/FLOATING filter tabs + search; click-to-expand matched articles.
...
const REMKNO_RE =
  /\bremkno\b|\breminders?.knowledge\b|\bknowledge.?reminders?\b|\bknowledge.?backed.?reminders?\b|\bfloating.?notes?\b|\breminder.?articles?\b|\breminders?.with.?knowledge\b|\breminder.?knowledge.?coverage\b/i;

export function isRemknoQuery(text) {
  return REMKNO_RE.test(text || "");
}
...
function crossRef(reminders, articles) {
  return reminders.map((rem) => {
    const haystack = `${rem.content} ${(rem.tags || []).join(" ")}`;
    const matches = articles
      .map((art) => {
        const hits = overlap(haystack, `${art.title} ${art.summary} ${art.tags}`);
        return hits > 0 ? { ...art, hits } : null;
      })
      .filter(Boolean)
      .sort((a, b) => b.hits - a.hits)
      .slice(0, 5);
    return { ...rem, articles: matches, backed: matches.length > 0 };
  });
}

export async function buildRemknoScript() {
  try {
    const base = apiBase();
    const hdr = { Authorization: `Bearer ${API_KEY}` };
    const [remRes, artRes] = await Promise.all([
      fetch(`${base}/reminders/list`, { headers: hdr }),
      fetch(`${base}/knowledge/`,     { headers: hdr }),
    ]);
    const [remData, artData] = await Promise.all([remRes.json(), artRes.json()]);
    const reminders = normaliseReminders(remData);
    const articles  = normaliseArticles(artData);
    const rows      = crossRef(reminders, articles);
    ...
    const topFloating = rows
      ...
      .join("; ");
    return (
      `${backed} of ${rows.length} reminders are knowledge-backed (${pct}% coverage). ` +
      (floating > 0
        ? `${floating} floating reminder${floating !== 1 ? "s" : ""} have no matching knowledge article — uncontextualised notes: ${topFloating || "unknown"}.`
        : "All reminders are supported by knowledge articles.")
    );
  } catch {
    return "Unable to reach reminders or knowledge endpoints, sir.";
  }
}

export default function RemindersKnowledgeNexus() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [remData, artData] = await Promise.all([remRes.json(), artRes.json()]);
      setRows(crossRef(normaliseReminders(remData), normaliseArticles(artData)));
    } catch {
      /* non-fatal */
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jarvis:remkno-toggle", toggle);
    return () => window.removeEventListener("jarvis:remkno-toggle", toggle);
  }, []);
  ...
  const assess = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}/v1/jarvis/agent/chat`, {
        ...
        body: JSON.stringify({
          message:
            `Assess reminder-to-knowledge linkage: ${rows.length} reminders total, ` +
            `${backed.length} are knowledge-backed, ` +
            `${floating.length} are floating with no associated knowledge article. ` +
            `Top floating: ${floating.slice(0, 3).map((r) => r.content.slice(0, 40)).join("; ") || "none"}. ` +
            "Give a 2-sentence operational-memory and knowledge-coverage assessment with recommended action.",
        }),
      });
  ...
  const visible = rows
    .filter((r) => {
      if (tab === "KNOWLEDGE-BACKED") return r.backed;
  ...
  const ART_KIND_COLOR = {
    ARTICLE: CY, REPORT: GRN, NOTE: AMB, GUIDE: GRN,
    PROCEDURE: AMB, REFERENCE: CY,
  };

  const BTN_LEFT = 82_160;
  const BTN_STYLE = {
    position: "fixed",
    left: BTN_LEFT,
    bottom: 8,
  ...
  return (
    <>
      <button
        style={BTN_STYLE}
        onClick={() => setOpen((o) => !o)}
        title="Reminders × Knowledge Nexus (REMKNO)"
      >
        ◈ REMKNO
        {floating > 0 && (
      ...
      {open && (
        <div style={PANEL}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ color: CY, fontSize: 11, letterSpacing: 2 }}>REMINDERS × KNOWLEDGE NEXUS</span>
            <button
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, marginBottom: 10 }}>
            {[
              ...
              { label: "KNOWLEDGE-BACKED",  value: backed,      color: backed > 0 ? GRN : DIM },
          ...
          <div style={{ display: "flex", gap: 5, marginBottom: 8 }}>
            {["ALL", "KNOWLEDGE-BACKED", "FLOATING"].map((t) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  ...
                  <span style={{ color: rem.backed ? GRN : DIM, fontSize: 9 }}>
                    {rem.backed ? `${rem.articles.length} article${rem.articles.length !== 1 ? "s" : ""}` : "FLOATING"}
                  </span>
                </div>
                  ...
                  <div style={{ marginTop: 6, borderTop: `1px solid ${CY}22`, paddingTop: 6 }}>
                    {rem.backed ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {rem.articles.map((art) => (
                          <div key={art.id} style={{ background: "rgba(0,229,160,0.04)", border: `1px solid ${GRN}33`, borderRadius: 4, padding: "5px 7px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                              <span style={{ color: ART_KIND_COLOR[art.kind] || CY, fontSize: 9, border: `1px solid ${(ART_KIND_COLOR[art.kind] || CY)}44`, borderRadius: 3, padding: "1px 4px" }}>{art.kind}</span>
                              <span style={{ color: "#DCEBF5", fontSize: 10, flex: 1 }}>{art.title}</span>
                              <span style={{ color: DIM, fontSize: 9 }}>hits: {art.hits}</span>
                            </div>
                            {art.summary && (
                              <div style={{ color: DIM, fontSize: 9, marginTop: 3, lineHeight: 1.4 }}>{art.summary.slice(0, 100)}{art.summary.length > 100 ? "…" : ""}</div>
                            )}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{ color: DIM, fontSize: 10 }}>No knowledge articles matched this reminder — uncontextualised action item.</div>
                    )}
                  </div>
                )}
              </div>
