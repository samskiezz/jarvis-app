      ...
      <QueryClientProvider client={queryClientInstance}>
        <Router future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
          <AuthGate>
            ...
            {/* F288 WorkshopAnalyticsPanel — ad-hoc analytics over live ontology objects;
            ...
            {/* F356 (overnight 2026-07-27): SwarmJobAnomalyCorrelator — ◈ SJAN button (left:634560, bottom:8, zIndex:232);
                parallel-fetches /entities/SwarmJob + /v1/jarvis/analytics/anomalies?limit=30 every 90 s;
                keyword-correlates each swarm job (name/kind/target/description/type/status) against anomaly metric names
                to surface EXPOSED (anomaly match found) vs CLEAR; stat tiles (swarm-jobs/anomalies/exposed/clear);
                ALL/EXPOSED/CLEAR filter tabs + text search; expand → matched anomaly chips with severity badge + z-score + score bars;
            ...
            {/* F359 (overnight 2026-07-27): IntelProfileInvestigationCorrelator — ◈ IPINV button (left:643680, bottom:8, zIndex:235);
                parallel-fetches /entities/IntelProfile + /v1/investigations every 90 s;
                keyword-correlates each intel profile (name/subject/description/category/nationality) against investigation
                titles/descriptions to surface LINKED (investigation connection found) vs UNLINKED (no investigative context);
                stat tiles (intel-profiles/investigations/linked/unlinked); ALL/LINKED/UNLINKED filter tabs + text search;
                expand → matched investigation chips with seed/annotation counts + score bars;
            ...
            {/* F375 (overnight 2026-07-28): OpsEventAnomalyCorrelator — ◈ OEAN button (left:716080, bottom:8, zIndex:251);
                parallel-fetches /v1/ops/events + /v1/jarvis/analytics/anomalies?limit=30 every 90 s;
                keyword-correlates each ops event (event_type/title/service/severity) against anomaly metrics to surface
                EXPOSED (anomaly-backed) vs CLEAR; stat tiles (ops-events/anomalies/exposed/clear);
                ALL/EXPOSED/CLEAR filter tabs + text search; expand event → severity/service chips + matched anomaly chips
                with z-score + score bars; red badge=exposed count, green=all-clear; ▶ ASSESS → /v1/jarvis/agent/chat
                2-sentence ops anomaly brief + TTS via jarvis:speak-dossier; isOeanQuery+buildOeanScript wired JarvisBrain;
            ...
            {/* F377 (overnight 2026-07-28): DatasetSpecCoverage — ◈ DSPC button (left:725200, bottom:8, zIndex:253);
                parallel-fetches /v1/datasets + /v1/spec/list every 90 s;
                keyword-correlates each dataset (name/description/kind/source) against spec entries (title/body_md/kind)
                to surface DOCUMENTED (spec found) vs UNDOCUMENTED (no spec backing);
                stat tiles (datasets/specs/documented/undocumented); ALL/DOCUMENTED/UNDOCUMENTED filter tabs + search;
                expand dataset → matched spec chips with status badge + score bars;
                amber badge=undocumented count, green=all documented; ▶ ASSESS → /v1/jarvis/agent/chat
                2-sentence dataset-spec coverage brief + TTS via jarvis:speak-dossier;
                isDspcQuery+buildDspcScript wired JarvisBrain;
