...
def _execute_connector(*args, **kwargs):
    source_id = kwargs.pop("source_id", "websocket_stream")
    url = kwargs.pop("url", kwargs.pop("endpoint", kwargs.pop("base_url", "")))
    return generic_connector("websocket_stream", source_id=source_id, url=url, **kwargs)

def run(*args, **kwargs):
    return _execute_connector(*args, **kwargs)

def parse(*args, **kwargs):
    return to_envelope(_execute_connector(*args, **kwargs), record_type="websocket_stream_connector_result")
